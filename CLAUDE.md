...
**Community translation reference** (`../wh-glossary/`, separate repo — clone it next to this one): 34k EN→RU pairs the RU fan-translation scene uses, extracted from the guide docx. **Grep it, never read it whole** — `grep -iP '^Kauyon\t' ../wh-glossary/terms/*.tsv`; conventions in `../wh-glossary/conventions.md`. It's a Black Library literary-translation guide, so it does **not** override this project's rule that unit/detachment/stratagem names and ALL-CAPS keywords stay English — use it for **flavour prose and transliteration**. Settled cases: `Kauyon` → «кауйон» (no apostrophe), `Mont’ka` → «монт’ка», `T'au` → «т’ау» (typographic ’ throughout).
