...
const KNOWN_EQUIVALENT = new Map([
  ...
  ['orks|Waaagh!', 'the flagged wording ("you can call a Waaagh!") is the COMBAT PATROL publication\'s Waaagh!, which appdata still ships alongside the codex one — and wh11ed carries it, verbatim, in src/data/combatPatrol.js\'s \'Ardmob box, which this script does not read (it searches the faction file only). Codex: Orks replaced that rule with War Cry, and orks.js states the codex version'],
