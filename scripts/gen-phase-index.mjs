...
for (const group of factionGroups) {
  for (const f of group.factions) {
    ...
    // `at` is the anchor on the faction page — the reminder is a name, and a name the reader
    // cannot get to is half a reminder. The ids are the same ones the search index anchors to
    // (`armyRule.id`, `<detId>-rule`), so the two land in the same place.
    const army = entryOf(faction.armyRule?.name, faction.armyRule?.body)
    if (army) { rec.army = { ...army, at: faction.armyRule?.id || null }; rules++ }
    ...
    for (const det of faction.detachments || []) {
      ...
      // `id` travels too: the faction page renders only the ACTIVE detachment, so following the
      // link means selecting it first (useFactionChoice), exactly as a search result does.
      if (e) { (rec.dets ??= {})[det.name] = { ...e, id: det.id, at: `${det.id}-rule` }; rules++ }
    }
...
// at?: anchor id, id?: detachment id } — p/s are exactly the arguments usableInSlot(p, s, phase,
// mine) takes. No rule TEXT: the block shows a name and links to where the text already lives. See the generator's header for what is left out
