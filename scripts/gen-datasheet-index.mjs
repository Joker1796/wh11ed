...
for (const group of factionGroups) {
  for (const f of group.factions) {
    ...
    out.push([f.slug, f.name, units.map((u) => [u.id, u.name, aliasesFor(u.name, aliases[u.id]), legacy[u.id] || null])])
  }
}
...
// [slug, factionName, [[unitId, unitName, aliasesRu, legacy], …]]. aliasesRu (RU nicknames) and
// legacy (retired Legends unit names proxied onto this sheet) are null (not an empty array —
...
console.log(`gen-datasheet-index: ${out.length} factions, ${total} units (${withAliases} with aliasesRu, ${withLegacy} with legacy names) → src/data/datasheetIndex.js`)
