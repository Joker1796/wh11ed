...
for (const group of factionGroups) {
  for (const f of group.factions) {
    ...
    out.push([f.slug, f.name, units.map((u) => [u.id, u.name, aliasesFor(u.name, aliases[u.id])])])
  }
}
...
// [slug, factionName, [[unitId, unitName, aliasesRu], …]]. aliasesRu is null (not an empty
// array — JSON.stringify can't omit an array slot) for the vast majority of units that carry
// none. Loaded on demand by useSearch.js (dynamic import) so it never rides in the entry bundle.
export const datasheetIndex = ${JSON.stringify(out)}
`
writeFileSync(join(ROOT, 'src/data/datasheetIndex.js'), body)
console.log(`gen-datasheet-index: ${out.length} factions, ${total} units (${withAliases} with aliasesRu) → src/data/datasheetIndex.js`)
