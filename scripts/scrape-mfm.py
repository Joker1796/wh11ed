...
OUT  = os.path.join(HERE, '..', 'src', 'data', 'mfmFactions.js')
CACHE = '/tmp/mfm-pages'
...
def emit(factions):
    L = ["// Munitorum Field Manual v%s — points values for Warhammer 40,000 (11th ed)." % MFM_VERSION,
    ...
    for f in factions:
        ...
        if f['units']:
            L.append("    units: [")
            L += [emit_unit(u, 6) + "," for u in f['units']]
            L.append("    ],")
        else: L.append("    units: [],")
        if f['subfactions']:
            ...
            for s in f['subfactions']:
                ...
                L += [emit_unit(u, 10) + "," for u in s['units']]
                L += ["        ],", "      },"]
            L.append("    ],")
...
if __name__ == "__main__":
    ...
    for slug in SLUGS:
        ...
        print(f"  {slug:22s} det={len(f['detachments']):2d} units={len(f['units']):3d}"
              ...
              file=sys.stderr)
        factions.append(f)
    open(os.path.normpath(OUT), 'w', encoding="utf-8").write(emit(factions))
    tot = sum(len(f['units']) + sum(len(s['units']) for s in f['subfactions']) for f in factions)
    print(f"Wrote {os.path.normpath(OUT)} — {len(factions)} factions, {tot} units", file=sys.stderr)
