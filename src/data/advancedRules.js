      ...
      subsections: [
        {
          ...
          body: `Каждый раз, когда вы совершаете **[gloss:normal-move:обычный манёвр]** или **[gloss:advance-move:продвижение]** отрядом, модели МОНСТР/ТЕХНИКА (MONSTER/VEHICLE) в этом отряде могут проходить сквозь дружественные и вражеские модели (кроме других моделей МОНСТР/ТЕХНИКА).`,
        },
        {
          id: 'section-17-02',
          sectionNum: '17.02',
...
При повороте модели РАМА как части манёвра, если у этой модели нет базы, поверните её на любой угол вокруг центральной оси, сохраняя вертикальное положение.`,
        },
        {
          ...
          body: `В вашу фазу стрельбы вражеские отряды МОНСТР/ТЕХНИКА (MONSTER/VEHICLE), находящиеся **[gloss:engaged:в ближнем бою]**, могут быть выбраны целью дальних атак (кроме атак оружием [BLAST]).
...
▪ В этом ТРАНСПОРТЕ достаточно оставшейся **[gloss:transport-capacity:вместимости транспорта]** для каждой модели этого отряда.
...
▪ **Тактическая высадка (Tactical Disembark):** В противном случае, если этот ТРАНСПОРТ **[gloss:remain-stationary:оставался неподвижным]** или ещё не был **[gloss:selected-to-move:выбран для манёвра]** в эту фазу, и вы можете расставить свой отряд, как описано ниже, вы должны выбрать этот режим.
...
◈ ВО ВРЕМЯ ДВИЖЕНИЯ | Расставьте каждую модель вашего отряда целиком в пределах **[gloss:set-up-distance:расстояния расстановки]** от этого ТРАНСПОРТА.
▪ **Стремительная высадка** (после **манёвра прибытия**): каждая модель должна следовать тем же правилам и ограничениям, которым должен был следовать ТРАНСПОРТ при отыгрыше этого манёвра прибытия.
▪ **Боевая высадка:** каждая модель может быть расставлена **[gloss:engaged:в ближнем бою]** с одним или несколькими вражескими отрядами, **находящимися в ближнем бою** с ТРАНСПОРТОМ.
◈ ПОСЛЕ ДВИЖЕНИЯ
...
▪ Целиком в пределах **[gloss:set-up-distance:расстояния расстановки]** от этого ТРАНСПОРТА и как можно ближе к нему.
▪ **Или:** Если это невозможно, оставаясь **[gloss:unengaged:не связанным боем]**, расставьте эту модель целиком в пределах **расстояния расстановки** от этого ТРАНСПОРТА, как можно ближе к нему, и **[gloss:engaged:связанной боем]**.
...
Правила, срабатывающие при **[gloss:destroyed:уничтожении]** отряда, срабатывают только при **уничтожении** последней модели, начавшей битву в составе **объединённого** отряда.`,
        },
        {
          ...
          example: `**[gloss:attached-unit:Объединённый]** отряд содержит модель **[gloss:leader:лидера]** с ключевым словом ПСАЙКЕР (PSYKER). Пока эта модель является частью этого отряда, этот отряд имеет ключевое слово ПСАЙКЕР, даже если модели **[gloss:bodyguard:телохранителей]** не имеют этого ключевого слова. Если этот отряд атакован оружием со способностью [АНТИ-ПСАЙКЕР 4+] (ANTI-PSYKER 4+), любой неизменённый (unmodified) [gloss:wound-roll:бросок на ранение] 4+ против этого отряда является **[gloss:critical-wound:критическим ранением]**, даже если сама атака не распределяется на модель **лидера**.`,
        },
        {
          id: 'section-19-04',
          sectionNum: '19.04',
...
На отряд может одновременно действовать более одной **способности ауры**, но если отряд находится в зоне действия одной и той же **способности ауры** более одного раза, эта **способность ауры** применяется к нему только один раз.`,
        },
        {
          ...
          body: `Некоторые способности общие для каждого отряда, принадлежащего к определённой фракции, — это **[gloss:faction-abilities:фракционные способности]** (также известные как **[gloss:army-rules:армейские правила]**), и они перечислены в разделе «Фракционные способности» карточки данных.
