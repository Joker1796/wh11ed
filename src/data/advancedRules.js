...
До битвы, на шаге «Объявление боевых построений» (Declare Battle Formations step), ваши отряды могут начать погружёнными в любую дружественную модель ТРАНСПОРТ с достаточной оставшейся **вместимостью транспорта** для всего отряда.`,
        },
        {
          ...
          sectionNum: '18.02',
          title: 'Погрузка',
          body: `После начала первого боевого раунда (battle round) дружественный отряд может погрузиться в дружественную модель ТРАНСПОРТ (TRANSPORT) после совершения **обычного движения** (normal move), **ускоренного движения** (advance move) или **отступления** (fall-back move), если применяются **все** следующие условия:
...
Когда отряд грузится, активный игрок убирает его с поля боя — теперь он погружён в этот ТРАНСПОРТ и не находится на поле боя.`,
        },
        {
          ...
          sectionNum: '18.03',
          title: 'Высадка',
          body: `В фазу движения (Movement phase) активного игрока каждый дружественный отряд, погружённый в модель ТРАНСПОРТ (TRANSPORT), может высадиться из неё, совершив **движение высадки** (disembark move) (см. ниже).

Если модель ТРАНСПОРТ **уничтожена** (destroyed), до её удаления с поля боя активный игрок должен совершить **экстренное движение высадки** (emergency disembark move) (см. напротив) каждым отрядом, погружённым в неё.`,
        },
        {
          id: 'section-18-04',
          sectionNum: '18.04',
...
▪ Этот ТРАНСПОРТ (TRANSPORT) не совершал **ускоренного движения** (advance move) или **отступления** (fall-back move) в эту фазу.
...
▪ **Стремительная высадка (Rapid Disembark):** Если этот ТРАНСПОРТ совершил **обычное движение** (normal move) или **движение прибытия** (ingress move) в эту фазу, вы должны выбрать этот режим.
▪ **Тактическая высадка (Tactical Disembark):** В противном случае, если этот ТРАНСПОРТ **оставался неподвижным** (remained stationary) или ещё не был **выбран для движения** (selected to move) в эту фазу, и вы можете расставить свой отряд, как описано ниже, вы должны выбрать этот режим.
...
▪ **Боевая высадка:** ваш отряд находится **в боевом шоке** (battle-shocked) и до конца хода не может объявлять атаку.`,
        },
        {
          id: 'section-18-05',
          sectionNum: '18.05',
...
◈ ПОСЛЕ ДВИЖЕНИЯ | Ваш отряд находится **в боевом шоке** (battle-shocked) и до конца хода не может объявлять атаку.`,
        },
      ],
    },
    {
...
▪ **Переброшенный** отряд, расставленный в том же ходу, в котором он совершил **ускоренное движение** (advance move), **отступление** (fall-back move) или **движение высадки** (disembark move), всё равно считается совершившим **ускоренное движение**, **отступление** или **движение высадки** в этот ход.
▪ Когда они убираются с поля боя, любые правила, действующие на такие отряды в течение определённого срока или при определённых обстоятельствах, продолжают действовать на них, пока этот срок и/или обстоятельства применяются.`,
          example: `Отряд, находившийся в зоне действия **способности ауры** (aura ability) при удалении с поля боя, больше не будет подвержен этой **способности ауры**, если при совершении **движения прибытия** (ingress move) он больше не находится в её зоне действия; однако отряд, находившийся **в боевом шоке** (battle-shocked) при удалении с поля боя, всё равно будет **в боевом шоке**, если совершит **движение прибытия** в тот же ход.`,
        },
        {
          id: 'section-20-03',
          sectionNum: '20.03',
          title: 'Прибытие из стратегического резерва',
          body: `Для прибытия на поле боя каждый отряд **стратегического резерва** (strategic reserves) должен совершить **движение прибытия** (ingress move) (см. ниже). Если не указано иное, они могут сделать это только начиная со второго боевого раунда (battle round).

В конце третьего боевого раунда, если не указано иное, все отряды **стратегического резерва**, не совершившие одно или несколько **движений прибытия**, **уничтожаются** (destroyed) со следующими исключениями:
▪ Отряды, погружённые в ТРАНСПОРТЫ (TRANSPORTS), которые совершили **движение прибытия** во время битвы.
...
▪ **До третьего боевого раунда:** при этом ни одна модель не может быть расставлена в зоне развёртывания (deployment zone) противника.
◈ ПОСЛЕ ДВИЖЕНИЯ | Если не указано иное, до начала следующей фазы атаки (Charge phase) ваш отряд не может совершать другие виды движения.`,
        },
      ],
    },
    {
      ...
      subsections: [
        {
          ...
          body: `Некоторые правила позволяют отряду совершить **стремительное движение** (surge move), как описано ниже.`,
        },
        {
          id: 'section-21-02',
          sectionNum: '21.02',
...
◈ ПЕРЕД ДВИЖЕНИЕМ | Выберите ближайший вражеский отряд в качестве **цели стремительного движения** (surge target).
◈ ВО ВРЕМЯ ДВИЖЕНИЯ
▪ Каждая модель должна завершить своё движение **в ближнем бою** (engaged) с **целью стремительного движения**, если это возможно.
...
[img:/images/surge/making-a-surge-move-ru.png|Выполнение хода рывка — допустимые цели и движение]`,
        },
        {
          ...
          body: `Модели с ключевым словом FLY могут **подниматься в небо** (take to the skies) при совершении **обычного движения** (normal move), **ускоренного движения** (advance move), **отступления** (fall-back move) или **движения в атаку** (charge move). Каждый раз, когда ЛЕТЯЩИЙ (FLYING) отряд выбирается для совершения такого движения, до движения любых моделей в этом отряде активный игрок может объявить, что он будет **подниматься в небо**. Если он это делает, при разрешении этого движения:
    ...
    },
    {
      ...
      subsections: [
        ...
        },
        {
          ...
          sectionNum: '23.02',
          title: 'Движение',
          body: `▪ Отряды ВОЗДУШНЫХ СУДОВ (AIRCRAFT) имеют право только на **движение прибытия** (ingress move) (20.04); они **не** имеют права на другие типы движения.
...
▪ Каждый раз, когда отряд совершает **сближение** (pile-in), **консолидацию** (consolidation) или **стремительное движение** (surge move), если этот отряд не может FLY, при совершении этого движения игнорируйте отряды ВОЗДУШНЫХ СУДОВ для целей выбора вражеских отрядов и определения ближайшего вражеского отряда.
▪ Нахождение **в ближнем бою** (engaged) исключительно с одним или несколькими отрядами ВОЗДУШНЫХ СУДОВ не препятствует праву отряда совершать **обычное движение** (normal move) или **ускоренное движение** (advance move).`,
        },
        {
          id: 'section-23-03',
          sectionNum: '23.03',
        ...
        },
        {
          ...
          body: `▪ Отряды ВОЗДУШНЫХ СУДОВ (AIRCRAFT) **не могут** объявлять атаку и могут наносить удары в ближнем бою только против ЛЕТЯЩИХ (FLYING) отрядов.
▪ Только ЛЕТЯЩИЕ отряды могут выбирать отряды ВОЗДУШНЫХ СУДОВ в качестве **цели атаки** (charge target), и только ЛЕТЯЩИЕ модели могут наносить удары в ближнем бою, нацеленные на отряды ВОЗДУШНЫХ СУДОВ.`,
        },
      ],
    },
  ],
