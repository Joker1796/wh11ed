...
[img:/images/monsters/engaged-monsters_vehicles-shooting.png]`,
...
[img:/images/surge/making-a-surge-move.png]`,
        },
        {
          id: 'section-21-03',
          sectionNum: '21.03',
...
[img:/images/surge/taking-to-the-skies.png]`,
...
[img:/images/fire/plunging-fire.png]`,
      ...
      subsections: [
        {
          ...
          body: `Каждый раз, когда вы совершаете **обычное движение** или **ускоренное движение** отрядом, модели МОНСТР/ТЕХНИКА в этом отряде могут проходить сквозь дружественные и вражеские модели (кроме других моделей МОНСТР/ТЕХНИКА).`,
        },
        {
          id: 'section-17-02',
          sectionNum: '17.02',
          title: 'Рама',
          body: `Некоторые модели не имеют подставки; многие из них — модели МОНСТР/ТЕХНИКА. Такие модели имеют ключевое слово РАМА, как и некоторые другие крупные модели. Каждый раз, когда правило ссылается на положение модели относительно чего-либо ещё на поле боя (например, при измерении расстояний), если эта модель имеет ключевое слово РАМА, если не указано иное, измеряйте до ближайшей точки этой модели и от неё (не обязательно от подставки, если она есть).

При повороте модели РАМА как части движения, если у этой модели нет подставки, поверните её на любой угол вокруг центральной оси, сохраняя вертикальное положение.`,
        },
        {
          id: 'section-17-03',
          sectionNum: '17.03',
          title: 'Стрельба по связанным монстрам и технике',
          body: `В вашу фазу стрельбы вражеские отряды МОНСТР/ТЕХНИКА, находящиеся **в ближнем бою**, могут быть выбраны целью дальних атак.

Каждый раз, когда модель совершает дальнюю атаку, нацеленную на такой отряд, вычтите 1 из **броска на попадание** (кроме атак оружием [БЛИЖНЯЯ СТРЕЛЬБА], совершаемых моделями из отряда, **находящегося в ближнем бою** с целью).

[img:/images/monsters/engaged-monsters_vehicles-shooting-ru.png]`,
          note: `Отряд, **находящийся в ближнем бою** с вражеским отрядом МОНСТР/ТЕХНИКА, всё равно **не может стрелять**, а значит, не может совершать дальние атаки против отряда МОНСТР/ТЕХНИКА, если только он **не может стрелять** будучи **в ближнем бою** (например, потому что использует **ближнюю стрельбу**).`,
        },
      ],
    },
    {
      ...
      title: 'Транспорты',
      page: 64,
      description: 'Модели ТРАНСПОРТ могут перевозить пассажиров по полю боя. Этот раздел объясняет, как использовать такие модели в ваших играх и как отряды могут погружаться в них и высаживаться из них.',
      subsections: [
        {
          id: 'section-18-01',
          sectionNum: '18.01',
          title: 'Вместимость транспорта',
          body: `Модели ТРАНСПОРТ имеют **вместимость транспорта**, указанную на их карточке данных. Это определяет тип и максимальное количество дружественных моделей, **которые могут погрузиться** в них. Более одного отряда может быть погружено в одну модель ТРАНСПОРТ одновременно при наличии достаточной **вместимости транспорта**.

До битвы, на шаге «Объявление боевых построений», ваши отряды могут начать погружёнными в любую дружественную модель ТРАНСПОРТ с достаточной оставшейся **вместимостью транспорта** для всего отряда.`,
        },
        {
          id: 'section-18-02',
          sectionNum: '18.02',
          title: 'Погрузка',
          body: `После начала первого боевого раунда дружественный отряд может погрузиться в дружественную модель ТРАНСПОРТ после совершения **обычного движения**, **ускоренного движения** или **отступления**, если применяются **все** следующие условия:
...
▪ Этот отряд **может погрузиться** в этот ТРАНСПОРТ, как описано на его карточке данных.
▪ В этом ТРАНСПОРТЕ достаточно оставшейся **вместимости транспорта** для каждой модели этого отряда.
...
Когда отряд грузится, активный игрок убирает его с поля боя — теперь он погружён в этот ТРАНСПОРТ и не находится на поле боя.`,
        },
        {
          ...
          sectionNum: '18.03',
          title: 'Высадка',
          body: `В фазу движения активного игрока каждый дружественный отряд, погружённый в модель ТРАНСПОРТ, может высадиться из неё, совершив **движение высадки** (см. ниже).

Если модель ТРАНСПОРТ **уничтожена**, до её удаления с поля боя активный игрок должен совершить **экстренное движение высадки** (см. напротив) каждым отрядом, погружённым в неё.`,
        },
        {
          id: 'section-18-04',
          sectionNum: '18.04',
          title: 'Движение высадки',
...
▪ Этот ТРАНСПОРТ не совершал **ускоренного движения** или **отступления** в эту фазу.
◈ ЭФФЕКТ | Ваш отряд расставляется, как описано в «Расстановке» (03.02).
◈ ПЕРЕД ДВИЖЕНИЕМ | Выберите **режим высадки** в следующем порядке:
▪ **Стремительная высадка:** Если этот ТРАНСПОРТ совершил **обычное движение** или **движение прибытия** в эту фазу, вы должны выбрать этот режим.
▪ **Тактическая высадка:** В противном случае, если этот ТРАНСПОРТ **оставался неподвижным** или ещё не был **выбран для движения** в эту фазу, и вы можете расставить свой отряд, как описано ниже, вы должны выбрать этот режим.
▪ **Боевая высадка:** В противном случае вы должны выбрать этот режим. Совершите **бросок на опасность** (06.03) за каждую модель вашего отряда.
◈ ВО ВРЕМЯ ДВИЖЕНИЯ | Расставьте каждую модель вашего отряда целиком в пределах **расстояния расстановки** от этого ТРАНСПОРТА.
▪ **Стремительная высадка** (после **движения прибытия**): каждая модель должна следовать тем же правилам и ограничениям, которым должен был следовать ТРАНСПОРТ при выполнении этого движения прибытия.
▪ **Боевая высадка:** каждая модель может быть расставлена **в ближнем бою** с одним или несколькими вражескими отрядами, **находящимися в ближнем бою** с ТРАНСПОРТОМ.
◈ ПОСЛЕ ДВИЖЕНИЯ
▪ **Стремительная высадка:** до конца хода ваш отряд не может объявлять атаку.
▪ **Тактическая высадка:** выберите ваш отряд для совершения **обычного движения** или **ускоренного движения**.
▪ **Боевая высадка:** ваш отряд находится **в боевом шоке** и до конца хода не может объявлять атаку.`,
        },
        {
          id: 'section-18-05',
          sectionNum: '18.05',
          title: 'Экстренное движение высадки',
          body: `◈ РАССТОЯНИЕ РАССТАНОВКИ | 6"
◈ УСЛОВИЕ | Ваш отряд погружён в модель ТРАНСПОРТ, которая только что была **уничтожена**.
◈ ЭФФЕКТ | Ваш отряд расставляется, как описано в «Расстановке» (03.02).
◈ ПЕРЕД ДВИЖЕНИЕМ | Совершите **бросок на опасность** (06.03) за каждую модель вашего отряда.
◈ ВО ВРЕМЯ ДВИЖЕНИЯ | Расставьте каждую модель вашего отряда целиком в пределах **расстояния расстановки** от этого ТРАНСПОРТА и как можно ближе к нему. Каждая модель, которую невозможно расставить таким образом, **уничтожается**.
◈ ПОСЛЕ ДВИЖЕНИЯ | Ваш отряд находится **в боевом шоке** и до конца хода не может объявлять атаку.`,
        },
      ],
    },
    {
      ...
      subsections: [
        {
          ...
          body: `Некоторые отряды имеют способность **Лидер** или **Поддержка**, указанную на их карточке данных. Такие отряды известны как отряды **лидеров** и отряды **поддержки** соответственно. Обе эти способности позволяют таким отрядам **вести** другие дружественные отряды (известные как отряды **телохранителей**) для создания **объединённых** отрядов. **Объединённый** отряд является единым отрядом для всех игровых целей. Отряды **лидеров** и **поддержки** могут вести только определённые отряды **телохранителей**, как указано в приложении Warhammer 40,000.

До битвы, на шаге «Формирование армий», для каждого отряда **лидера** и **поддержки** в вашей армии вы можете выбрать один дружественный отряд **телохранителей**, которым он может **командовать**. Этот отряд будет **вести** этот отряд **телохранителей** в битве и создаст с ним **объединённый** отряд.
...
Если не указано иное, каждый отряд **телохранителей** может иметь только один присоединённый отряд **лидера** и один отряд **поддержки**.`,
        },
        {
          ...
          body: `Каждый раз, когда атака нацелена на **объединённый** отряд, если он содержит одну или несколько моделей **телохранителей**, используйте наивысшую характеристику **Ж** моделей **телохранителей** в этом отряде при разрешении этой атаки, даже если отряд **лидера/поддержки** в этом **объединённом** отряде имеет другую характеристику **Ж**. Если отряд содержит только модели **лидера/поддержки**, вместо этого используйте наивысшую характеристику **Ж** этих моделей.

Правила, срабатывающие при **уничтожении** отряда, срабатывают только при **уничтожении** последней модели, начавшей битву в составе **объединённого** отряда.`,
        },
        {
          id: 'section-19-03',
          sectionNum: '19.03',
          title: 'Ключевые слова в объединённых отрядах',
          body: `**Объединённый** отряд имеет все ключевые слова всех составляющих его отрядов. Таким образом, на **объединённый** отряд влияет любое правило, применяемое к отрядам с любым из этих ключевых слов. Обратите внимание, что модели в **объединённом** отряде не получают ключевых слов других моделей в этом отряде, которых у них ещё нет. Помните, что атаки нацелены на отряды, а не на модели.`,
          example: `**Объединённый** отряд содержит модель **лидера** с ключевым словом ПСАЙКЕР. Пока эта модель является частью этого отряда, этот отряд имеет ключевое слово ПСАЙКЕР, даже если модели **телохранителей** не имеют этого ключевого слова. Если этот отряд атакован оружием со способностью [АНТИ-ПСАЙКЕР 4+], любой неизменённый бросок на ранение 4+ против этого отряда является **критическим ранением**, даже если сама атака не распределяется на модель **лидера**.`,
        },
        {
          id: 'section-19-04',
          sectionNum: '19.04',
          title: 'Способности в объединённых отрядах',
          body: `Способности/правила, влияющие на одну конкретную модель (например, от **улучшения** или предмета снаряжения), применяются только к этой модели, даже если она является частью **объединённого** отряда.

В противном случае способности/правила, влияющие на отряд (или модели в нём), применяются к каждой модели в **объединённом** отряде до **уничтожения** источника этой способности/правила, как показано в таблице ниже.
...
Во всех вышеперечисленных случаях, если эта последняя модель была **уничтожена** в результате атаки, способность, которую она предоставляла **объединённому** отряду, действует до тех пор, пока атакующий отряд не разрешит все свои атаки.`,
          note: `* Это означает, что отряды **лидеров/поддержки** продолжают пользоваться своими собственными способностями «пока эта модель ведёт отряд» даже после **уничтожения** их отряда **телохранителей**, при условии, что они начали битву в составе **объединённого** отряда. Если эти модели впоследствии будут **воскрешены**, эти способности снова будут применяться к их **объединённому** отряду.`,
        },
      ],
      abilitiesTable: {
        title: 'Способности в объединённых отрядах',
      ...
      subsections: [
        {
          ...
          body: `До битвы, на шаге «Объявление боевых построений», вы можете выбрать один или несколько дружественных отрядов (кроме УКРЕПЛЕНИЙ), чтобы поместить их в **стратегический резерв**. Вместо того чтобы расставлять эти отряды на поле боя во время развёртывания, отложите их в сторону — они являются отрядами **стратегического резерва** и прибудут позже в ходе битвы.

Если не указано иное, суммарная стоимость в очках всех ваших отрядов **стратегического резерва** (включая те, что погружены в ТРАНСПОРТЫ, сами помещённые в **стратегический резерв**) не может превышать 50% от вашего лимита очков для вашего размера битвы.`,
        },
        {
          id: 'section-20-02',
          sectionNum: '20.02',
          title: 'Переброшенные отряды',
          body: `Некоторые правила позволяют убирать отряды с поля боя и помещать их в **стратегический резерв** во время битвы. Отряды, использующие такие правила, известны как **переброшенные** отряды. В дополнение к любым другим правилам, применяемым к таким отрядам (например, где они могут или не могут прибыть), к ним применяются все следующие правила:
▪ При использовании в фазу движения такие правила могут применяться к отрядам, которые уже двигались в эту фазу.
▪ **Переброшенный** отряд, расставленный в том же ходу, в котором он совершил **ускоренное движение**, **отступление** или **движение высадки**, всё равно считается совершившим **ускоренное движение**, **отступление** или **движение высадки** в этот ход.
▪ Когда они убираются с поля боя, любые правила, действующие на такие отряды в течение определённого срока или при определённых обстоятельствах, продолжают действовать на них, пока этот срок и/или обстоятельства применяются.`,
          example: `Отряд, находившийся в зоне действия **способности ауры** при удалении с поля боя, больше не будет подвержен этой **способности ауры**, если при совершении **движения прибытия** он больше не находится в её зоне действия; однако отряд, находившийся **в боевом шоке** при удалении с поля боя, всё равно будет **в боевом шоке**, если совершит **движение прибытия** в тот же ход.`,
        },
        {
          id: 'section-20-03',
          sectionNum: '20.03',
          title: 'Прибытие из стратегического резерва',
          body: `Для прибытия на поле боя каждый отряд **стратегического резерва** должен совершить **движение прибытия** (см. ниже). Если не указано иное, они могут сделать это только начиная со второго боевого раунда.

В конце третьего боевого раунда, если не указано иное, все отряды **стратегического резерва**, не совершившие одно или несколько **движений прибытия**, **уничтожаются** со следующими исключениями:
▪ Отряды, погружённые в ТРАНСПОРТЫ, которые совершили **движение прибытия** во время битвы.
▪ **Переброшенные** отряды.`,
        },
        {
          id: 'section-20-04',
          sectionNum: '20.04',
          title: 'Движение прибытия',
          body: `◈ РАССТОЯНИЕ РАССТАНОВКИ | 6"
◈ УСЛОВИЕ | Ваш отряд находится в **стратегическом резерве** и не погружён в ТРАНСПОРТ, также находящийся в **стратегическом резерве**.
◈ ЭФФЕКТ | Ваш отряд расставляется, как описано в «Расстановке» (03.02).
...
▪ **До третьего боевого раунда:** при этом ни одна модель не может быть расставлена в зоне развёртывания противника.
◈ ПОСЛЕ ДВИЖЕНИЯ | Если не указано иное, до начала следующей фазы атак ваш отряд не может совершать другие виды движения.`,
        },
      ],
    },
    {
      id: '21',
      num: '21',
      title: 'Полёт и стремительное движение',
      page: 70,
      ...
      subsections: [
        {
          id: 'section-21-01',
          sectionNum: '21.01',
          title: 'Стремительное движение',
          body: `Некоторые правила позволяют отряду совершить **стремительное движение**, как описано ниже.`,
        },
        {
          id: 'section-21-02',
          sectionNum: '21.02',
          title: 'Стремительное движение',
          body: `◈ МАКСИМАЛЬНОЕ РАССТОЯНИЕ | Как указано в правиле, разрешающем этот тип движения.
◈ УСЛОВИЕ | Применяются все из следующего.
▪ Правило, разрешающее этот тип движения, было активировано.
▪ Ваш отряд не **в боевом шоке**.
▪ Ваш отряд **не в ближнем бою**.
▪ Ваш отряд не двигался в эту фазу.
◈ ЭФФЕКТ | Ваш отряд движется, как описано в «Движении» (03).
◈ ПЕРЕД ДВИЖЕНИЕМ | Выберите ближайший вражеский отряд в качестве **цели стремительного движения**.
◈ ВО ВРЕМЯ ДВИЖЕНИЯ
▪ Каждая модель должна завершить своё движение **в ближнем бою** с **целью стремительного движения**, если это возможно.
▪ Каждая модель, не способная завершить движение **в ближнем бою** с **целью стремительного движения**, должна завершить движение как можно ближе к **цели стремительного движения**.
◈ ПОСЛЕ ДВИЖЕНИЯ
...
[img:/images/surge/making-a-surge-move-ru.png]`,
        },
        {
          id: 'section-21-03',
          sectionNum: '21.03',
          title: 'Летящие модели',
          body: `Модели с ключевым словом FLY могут **подниматься в небо** при совершении **обычного движения**, **ускоренного движения**, **отступления** или **движения в атаку**. Каждый раз, когда ЛЕТЯЩИЙ отряд выбирается для совершения такого движения, до движения любых моделей в этом отряде активный игрок может объявить, что он будет **подниматься в небо**. Если он это делает, при разрешении этого движения:
▪ Вычтите 2" из **максимального расстояния**.
...
▫ Она может проходить сквозь все типы моделей (включая вражеские модели и модели МОНСТР/ТЕХНИКА).
▫ Она может двигаться горизонтально и вертикально через все категории **элементов ландшафта**.

[img:/images/surge/taking-to-the-skies-ru.png]`,
        },
      ],
    },
    {
      ...
      subsections: [
        {
          ...
          body: `Способности, влияющие на модели или отряды в пределах указанной дистанции, являются **способностями ауры** и помечаются словом «Аура».
...
На отряд может одновременно действовать более одной **способности ауры**, но если отряд находится в зоне действия одной и той же **способности ауры** более одного раза, эта **способность ауры** применяется к нему только один раз.`,
        },
        {
          ...
          body: `Некоторые способности общие для каждого отряда, принадлежащего к определённой фракции, — это **фракционные способности** (также известные как **армейские правила**), и они перечислены в разделе «Фракционные способности» карточки данных.

Если не указано иное, **фракционные способности** отряда применяются только в том случае, если **фракция армии**, выбранная вами при формировании армии, совпадает с ключевым словом фракции, указанным на карточке данных этого отряда.`,
        },
        {
          id: 'section-22-03',
          sectionNum: '22.03',
          title: 'Псайкерские способности',
          body: `Способности, помеченные словом «Псайкер», являются **псайкерскими способностями**. Если **псайкерская способность** заставляет модель потерять одно или несколько ранений, каждое из этих ранений считается нанесённым **псайкерской атакой** (это может быть важно для активации других правил).`,
        },
        {
          id: 'section-22-04',
          sectionNum: '22.04',
          title: 'Способности снаряжения',
          body: `Способности, получаемые, когда отряд (или одна из его моделей) имеет определённый предмет снаряжения, являются **способностями снаряжения** и перечислены в разделе «Способности снаряжения» карточки данных.
...
Если отряд имеет предмет снаряжения со **способностью снаряжения**, эта способность применяется к отряду. Если модель в составе отряда имеет предмет снаряжения со **способностью снаряжения**, эта модель является «носителем» этого предмета снаряжения, и способность действует до тех пор, пока эта модель не будет **уничтожена**.`,
        },
        {
          ...
          body: `Каждый раз, когда модель совершает дальнюю атаку, нацеленную на **видимый** отряд, содержащий одну или несколько моделей на **уровне земли**, если применяется одно или несколько следующих условий, улучшите характеристику **БС** этой атаки на 1:
▪ Атакующая модель находится на участке **элемента ландшафта** высотой 3" или более.
▪ Атакующая модель имеет ключевое слово ИСПОЛИНСКИЙ, и целевой отряд находится в пределах 12".

[img:/images/fire/plunging-fire-ru.png]`,
        },
      ],
    },
    {
      ...
      page: 74,
      description: 'ВОЗДУШНЫЕ СУДА выполняют уникальную, но ограниченную роль на полях сражений Warhammer 40,000. Мчась сквозь небеса, они сражаются друг с другом, обстреливают наземные силы или наносят бомбовые удары, находясь в постоянном движении.',
      subsections: [
        {
          id: 'section-23-01',
          sectionNum: '23.01',
          title: 'Развёртывание',
          body: `На шаге «Объявление боевых построений» все отряды ВОЗДУШНЫХ СУДОВ должны быть помещены в **стратегический резерв** (20.01).`,
        },
        {
          id: 'section-23-02',
          sectionNum: '23.02',
          title: 'Движение',
          body: `▪ Отряды ВОЗДУШНЫХ СУДОВ имеют право только на **движение прибытия** (20.04); они **не** имеют права на другие типы движения.
▪ В конце хода вашего противника все отряды ВОЗДУШНЫХ СУДОВ в вашей армии, находящиеся на поле боя, должны быть помещены в **стратегический резерв**.
▪ Каждый раз, когда отряд совершает любой вид движения, его модели могут проходить сквозь модели ВОЗДУШНЫХ СУДОВ.
▪ Каждый раз, когда отряд совершает **сближение**, **консолидацию** или **стремительное движение**, если этот отряд не может FLY, при совершении этого движения игнорируйте отряды ВОЗДУШНЫХ СУДОВ для целей выбора вражеских отрядов и определения ближайшего вражеского отряда.
▪ Нахождение **в ближнем бою** исключительно с одним или несколькими отрядами ВОЗДУШНЫХ СУДОВ не препятствует праву отряда совершать **обычное движение** или **ускоренное движение**.`,
        },
        {
          id: 'section-23-03',
          sectionNum: '23.03',
          title: 'Стрельба',
          body: `Правило «Навесной огонь» (22.05) не влияет на атаки, совершаемые отрядами ВОЗДУШНЫХ СУДОВ или нацеленные на них.`,
        },
        {
          id: 'section-23-04',
          sectionNum: '23.04',
          title: 'Атака и ближний бой',
          body: `▪ Отряды ВОЗДУШНЫХ СУДОВ **не могут** объявлять атаку и могут наносить удары в ближнем бою только против ЛЕТЯЩИХ отрядов.
▪ Только ЛЕТЯЩИЕ отряды могут выбирать отряды ВОЗДУШНЫХ СУДОВ в качестве **цели атаки**, и только ЛЕТЯЩИЕ модели могут наносить удары в ближнем бою, нацеленные на отряды ВОЗДУШНЫХ СУДОВ.`,
        },
      ],
    },
  ],
