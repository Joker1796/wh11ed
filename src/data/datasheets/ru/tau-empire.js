...
const SHIELD_GENERATOR = 'Носитель имеет инвульнерабельный спасбросок 4+.'
const BSS_BEARER =
...
const DS8_SUPPORT_TURRET =
  'В вашей фазе движения, если этот юнит остаётся неподвижным, до начала вашей следующей фазы движения его модель Fire Warrior Shas’ui снаряжается оружием support turret missile system.\n\n**Примечание разработчика:** положите рядом с этим юнитом жетон Support Turret, чтобы не забыть.'
const FORTIFICATION =
...
export default {
  ...
  'commander-farsight': {
    ...
    abilities: {
      ...
      "Puretide's Teachings":
        'Один раз за раунд боя вы можете выбрать юнит этой модели целью стратагемы. Если вы это делаете, уменьшите стоимость этого применения этой стратагемы на 1 CP.',
    },
    leader: { text: LEADER_TEXT },
  },
  ...
  'hammerhead-gunship': {
    ...
    abilities: {
      ...
      'Targeting Array':
        'Каждый раз, когда эта модель выбирается для стрельбы, при разрешении этих атак вы можете перебросить один бросок попадания или один бросок ранения.',
    },
  ...
  'kroot-carnivores': {
    ...
    abilities: {
      Fieldcraft:
        'В конце вашей фазы командования, если этот юнит находится в пределах дальности маркера цели, который вы контролируете, этот маркер цели остаётся под вашим контролем, даже если у вас нет моделей в пределах его дальности, до тех пор, пока ваш оппонент не станет контролировать его в начале или конце любого хода.',
    },
    options: [
  ...
  'kroot-trail-shaper': {
    ...
    abilities: {
      'Trail Finding':
        'Один раз за ход, когда вражеский юнит заканчивает обычный манёвр, продвижение или отступление в пределах 9" от юнита этой модели, если юнит этой модели не находится в дистанции ввязывания одного или более вражеских юнитов, он может совершить обычный манёвр до D6".',
      'Kroot Ambush':
  ...
  'pathfinder-team': {
    ...
    wargearAbilities: {
      'Grav-inhibitor Drone':
        'Вычтите 2 из бросков нападения, сделанных для любого вражеского юнита, объявляющего нападение на юнит носителя (это не складывается с другими уменьшениями того броска нападения).',
  ...
  'riptide-battlesuit': {
    ...
    abilities: {
      'Battlesuit Support System':
        'Носитель может стрелять в ход, в который он отступил.',
      'Nova Charge':
  ...
  'sky-ray-gunship': {
    ...
    abilities: {
      ...
      'Targeting Array':
        'Каждый раз, когда этот юнит выбирается для стрельбы, при разрешении этих атак вы можете перебросить один бросок попадания или один бросок ранения.',
    },
    damaged: { note: 'осталось 1–5 ран', text: dmgHit('1–5') },
    options: [
      '2 twin pulse carbines этой модели можно заменить на одно из следующего:\n▪ 2 accelerator burst cannons\n▪ 2 smart missile systems',
  ...
  'stealth-battlesuits': {
    ...
    wargearAbilities: {
      'Homing Beacon':
        'Один раз за битву вы можете применить стратагему Rapid Ingress за 0 CP. Цель должна быть выставлена в пределах 3" от юнита носителя и далее 9" от всех вражеских юнитов.\n\n**Примечание разработчика:** положите рядом с этим юнитом жетон Homing Beacon, убирая его, когда эта способность задействуется.',
    },
    options: [
  ...
  'sun-shark-bomber': {
    ...
    abilities: {
      'Pulse Bombs':
        'Каждый раз, когда эта модель заканчивает обычный манёвр, вы можете выбрать один вражеский юнит, над которым она пролетела во время этого манёвра, и бросить шесть D6: за каждый результат 4+ тот юнит получает 1 смертельную рану.',
    },
    damaged: { note: 'осталось 1–4 ран', text: dmgHit('1–4') },
    options: [
  ...
  'the-twin-lance': {
    ...
    abilities: {
      ...
      'Neocapacitor Shields':
        'В начале фазы нападения вашего оппонента вы можете выбрать один вражеский юнит (исключая юниты Monster и Vehicle) в пределах 12" от этого юнита. Тот юнит должен пройти проверку боевого шока и, до конца хода, вычесть 1 из бросков нападения, сделанных для него.',
      'Retro-thrusters':
  ...
  'tidewall-droneport': {
    ...
    abilities: {
      Droneport:
        'Каждый раз, когда этот Fortification выбирается для стрельбы, оружие его drone defender будет целиться и разрешать атаки против каждого вражеского юнита, что является допустимой целью для этого Fortification.',
