...
▪ Ни одна модель в этом юните не перемещалась более чем на 3" в этот ход.`,
    ...
    {
      // 24.21 [LANCE]
      flavor: 'Используя инерцию несущегося на полной скорости воина, копьевое оружие смертоносно в рывке.',
      fullText: `Каждый раз, когда атака совершается оружием **[LANCE]**, если атакующий юнит совершил **перемещение рывком** (charge move) в этот ход, добавьте 1 к **броску на ранение** (wound roll).`,
    },
    {
      // 24.22 Leader
  ...
  ru: [
    {
      ...
      a: 'Да. Даже если юнит не может совершать дистанционные атаки, он всё равно **может стрелять** (eligible to shoot), что может повлиять на его возможность использования других правил, например, начала **задания** (starting an action).',
    },
    {
    ...
    {
      q: 'Может ли юнит, который может совершить **рывок к врагу** (overrun fight), быть выбран для этого, если он **не может участвовать в ближнем бою** (eligible to fight)?',
      a: 'Нет. Иногда юнит может **связаться ближним боем** (engaged) после начала шага «Ближний бой» (Fight step), но затем стать **несвязанным** (unengaged) позже в этом шаге. Если это происходит, и он больше **не может участвовать в ближнем бою** (eligible to fight), он не может совершить **рывок к врагу** (overrun fight).',
    },
    {
