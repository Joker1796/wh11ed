...
Каждый раз, когда вы перемещаете модель по прямой линии, перемещайте её горизонтально по полю боя. Измеряйте расстояние от одной и той же точки на её базе в начале и в конце этого перемещения и прибавляйте это расстояние к любому другому расстоянию, которое она преодолела с момента начала перемещения её юнита. Пройденное расстояние не может превышать **максимальное расстояние**(maximum distance) для данного **типа перемещения**(move type).
