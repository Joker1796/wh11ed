...
export const changelog = [
  ...
  },
  {
    ...
    date: '2026-09-25',
    en: [
      { h: 'Roster builder: more room to work' },
      'On a phone the catalogue and the list now start right under the top bar. The list’s name moved to the settings, and the buttons {btn:gear} {btn:panes} at the bottom of the screen switch between settings and units. The faction rules open from {btn:book} next to the unit search. On a narrow phone Cancel is {btn:revert}. On a computer, Cancel and Save moved to the settings line.',
      { h: 'Roster builder: attaching from the squad' },
      'A squad’s card now lists the Leaders and Supports in the list that can join it, and they can be attached from there.',
      { h: 'Roster builder: shrinking a unit' },
      'When a unit gets smaller, the builder removes the wargear that no longer fits and says so. {key:Put back} undoes the change. In imported lists, wargear over the limit is marked in red.',
      { h: 'Roster builder: wargear that cannot be replaced' },
      'The builder now follows the “cannot be replaced” clause: when a swap locks a second weapon, that weapon can no longer be traded. This applies to 29 datasheets, including Raptors, Battle Sisters, Kasrkin and Chosen.',
      { h: 'Roster builder: chains of attached units' },
      'A unit in a chain of attachments (for example, Huron Blackheart → Masters of the Maelstrom → Chosen) is no longer shown twice. The whole chain is one block with one points total.',
      { h: 'List page' },
      'Attached units are shown under their block’s name with the points for the whole block, as in the builder. The cloud save message appears at the bottom of the screen and hides by itself. Cancel in the builder returns to the list’s page.',
      { h: 'Faction pages: Force Disposition colours' },
      'Detachments in the picker show their Force Disposition in its colour, under the price, as in the builder.',
      { h: 'Search' },
      'Results from the Event Companion’s Errata & FAQs and introduction, the terrain footprint table and Snap Shooting now open at the right place on the page, not at its top.',
      { h: 'Shared setup' },
      'The lobby’s questions close again with the ×, the Back button and the phone’s back gesture.',
    ],
    ru: [
      { h: 'Конструктор: больше места для работы' },
      'На телефоне каталог и список начинаются сразу под верхней панелью. Название ростера перенесено в настройки, а переключение между настройками и юнитами — на кнопки {btn:gear} {btn:panes} внизу экрана. Правила фракции открываются кнопкой {btn:book} рядом с поиском юнитов. На узком телефоне «Отмена» — это {btn:revert}. На компьютере «Отмена» и «Сохранить» перенесены в строку настроек.',
      { h: 'Конструктор: прикрепление со стороны отряда' },
      'В карточке отряда показаны Leader и Support из списка, которые могут к нему присоединиться. Прикрепить их можно прямо оттуда.',
      { h: 'Конструктор: уменьшение отряда' },
      'При уменьшении отряда конструктор снимает снаряжение, которое больше не помещается, и сообщает об этом. Кнопка {key:Вернуть} отменяет изменение. В импортированных списках превышение отмечается красным.',
      { h: 'Конструктор: оружие, которое нельзя заменить' },
      'Конструктор учитывает оговорку «cannot be replaced»: если обмен закрепляет второе оружие, заменить его больше нельзя. Правило действует для 29 датащитов, в том числе Raptors, Battle Sisters, Kasrkin и Chosen.',
      { h: 'Конструктор: цепочки прикреплённых отрядов' },
      'Отряд в цепочке прикреплений (например, Huron Blackheart → Masters of the Maelstrom → Chosen) больше не отображается дважды. Вся цепочка показывается одним блоком с общей суммой очков.',
      { h: 'Страница ростера' },
      'Прикреплённые отряды показаны под названием блока с суммой очков за весь блок, как в конструкторе. Сообщение о сохранении в облако появляется внизу экрана и скрывается само. Кнопка «Отмена» в конструкторе возвращает на страницу ростера.',
      { h: 'Страницы фракций: цвета Force Disposition' },
      'Детачменты в списке выбора показывают свою Force Disposition её цветом под ценой, как в конструкторе.',
      { h: 'Поиск' },
      'Результаты из Errata & FAQs и введения Event Companion, таблицы размеров террейна и правила Snap Shooting открываются на нужном месте страницы, а не в её начале.',
      { h: 'Совместная настройка' },
      'Вопросы лобби снова закрываются крестиком, кнопкой «Назад» и жестом «назад» на телефоне.',
    ],
  },
  {
    version: '2.7.0',
    date: '2026-09-24',
    en: [
      { h: 'Set the game up together' },
      'On the tracker page press “Shared game” and start a new one. You get a link, a QR code and a six-digit code. Your opponent joins from their own phone and fills in their own side: name, army, detachments, list, Force Disposition. The mission, the battlefield and the turn order stay with you. You start the game too. Only the one who creates it needs an account.',
      'Their side arrives whole, once they press Done. While you are on the armies step they can change it freely. After that they ask, and you allow it or not.',
      'In doubles one phone fills in one side — the one that opened it first. The partner waits and can take the editing over.',
      { h: 'Tracker: one button at the top' },
      'The top of the tracker page has one button: continue the game, continue the setup, or start a new one. The rest sit quietly under it.',
      { h: 'Combat Patrol: no list needed' },
      'The box is the army. So the button that attached a 2000-point list is gone from a Combat Patrol setup.',
      { h: 'Roster builder: the list is easier to read' },
      'Unit names and prices in the catalogue are now the same size as in the list beside it. They used to be noticeably smaller. The category headers are bigger too, and so are the headings in the faction rules. The filters moved into an icon next to the search and no longer take a line of their own.',
      { h: 'Roster builder: a unit you delete can be brought back' },
      'Delete one with the wrong finger and “Undo” appears at the bottom. The unit goes back where it stood, with the Leader that was attached to it. Until now it could not be brought back at all. The catalogue also lost its “−”: it removed a unit from the list while sitting one tap below the “+”. Copy and delete moved into the ⋮ menu on the unit’s own card.',
      { h: 'Roster builder: Cancel now cancels' },
      'The Cancel button at the bottom of the editor puts the list back the way you opened it. It used to just close the editor and keep every change. If you changed something, the app asks first and says what is about to go.',
      { h: 'Roster builder: a squad with a Leader is one block' },
      'A squad with characters attached has a header of its own now: a name, what the whole thing costs, and a fold. You can write your own name on it — whatever you call the squad while planning. Until you do, it is numbered. The army’s colour runs down the left edge of the block, from the header to the last character.',
      { h: 'Roster builder: you can see which squad you are attaching to' },
      'With two squads of the same datasheet in a list, “attach to unit” showed the same name twice in a row. Now the line under it says what makes them different: the squad’s name, how many models, the wargear you picked for it, who is attached already.',
      { h: 'Roster builder: the Setup tab says when something is left to choose' },
      'A list can be built by the rules and still have an open choice in its setup: no Force Disposition declared, or no detachment picked. The bottom of the screen showed a green tick, so you pressed Save and only found out there. An amber mark now sits on the Setup tab, and the tick at the bottom turns amber with it.',
      { h: 'Roster builder: weapon swaps the list did not know about' },
      'A Raptor who took a mutation kept the chainsword it was traded for. And the builder offered two mutations where one sword was left to trade. 29 swaps in 17 units read that way: the Deathwatch kill teams, Death Korps of Krieg, Noise Marines and others. The traded weapon now leaves the list, and you cannot trade away more than the unit carries.',
      { h: 'Roster builder: on a wide screen you can see what you type' },
      'On the light theme, on a wide screen, the text in the name field blended into its background. The faction, detachment, points and notes beside it did the same. They are ordinary fields now, and what you type shows.',
      { h: 'Windows no longer scroll the page to the top' },
      'You closed the opponent’s secondary picker and the page stood at the very top, their card far below it. Every dialog in the app did that since 2.6.4. Now you stay where you were.',
      { h: 'Thank you' },
      'Thank you to everyone who sends bug reports, and to everyone who suggests how to make things better. It helps a great deal — much of what is above came out of your messages.',
    ],
    ru: [
      { h: 'Настраиваем партию вместе' },
      'На странице трекера нажмите «Совместная игра» и начните новую. Вы получите ссылку, QR и шестизначный код. Соперник подключится со своего телефона и заполнит свою сторону: имя, армию, детачменты, ростер, Force Disposition. Миссия, поле и очерёдность хода остаются за вами. Партию тоже начинаете вы. Аккаунт нужен только тому, кто создаёт игру.',
      'Его сторона придёт целиком, когда он нажмёт «Готово». Пока вы на шаге «Армии», он меняет её свободно. Дальше он просит правку, а вы разрешаете или нет.',
      'В парной игре одну сторону заполняет один телефон — тот, что открыл её первым. Напарник ждёт и может забрать редактирование себе.',
      { h: 'Трекер: наверху одна кнопка' },
      'Наверху страницы трекера одна кнопка: продолжить партию, продолжить настройку или начать новую. Остальное стоит тихой строкой под ней.',
      { h: 'Combat Patrol: ростер не нужен' },
      'Коробка и есть армия. Поэтому в настройке Combat Patrol больше нет кнопки, которая прикрепляла список на 2000 очков.',
      { h: 'Конструктор: список стало легче читать' },
      'Названия юнитов и цены в каталоге теперь такого же размера, как в списке рядом. Раньше они были заметно мельче. Крупнее стали заголовки категорий и заголовки в правилах фракции. Фильтры переехали в иконку рядом с поиском и больше не занимают строку.',
      { h: 'Конструктор: удалённый юнит можно вернуть' },
      'Удалили не тем пальцем — внизу появится «Вернуть». Юнит встанет на своё место вместе с лидером, который к нему цеплялся. Раньше вернуть его было нельзя. У каталога заодно пропал «−»: он убирал юнит из списка, стоя на один тап ниже «+». Копирование и удаление переехали в меню ⋮ на карточке юнита.',
      { h: 'Конструктор: «Отмена» теперь отменяет' },
      'Кнопка «Отмена» внизу редактора возвращает список к тому виду, в котором вы его открыли. Раньше она просто закрывала редактор, а правки оставались. Если вы что-то меняли, приложение сначала спросит и покажет, что именно исчезнет.',
      { h: 'Конструктор: отряд с лидером — один блок' },
      'У отряда с прикреплёнными персонажами теперь своя шапка: имя, цена всего отряда и сворачивание. Имя можно написать своё — как вы зовёте отряд, когда планируете. Пока имени нет, стоит номер. Цвет армии идёт по левому краю блока, от шапки до последнего персонажа.',
      { h: 'Конструктор: видно, к какому отряду прикрепляете' },
      'Когда в списке два одинаковых отряда, выбор «Прикрепить к отряду» показывал два одинаковых названия подряд. Теперь под названием написано, чем они отличаются: имя отряда, сколько моделей, какое оружие вы ему выбрали, кто уже прикреплён.',
      { h: 'Конструктор: вкладка «Настройка» говорит, что там осталось выбрать' },
      'Список может быть собран по правилам, но в настройке остался невыбранный пункт: не заявлен Force Disposition или не выбран детачмент. Внизу экрана при этом стояла зелёная галочка — вы жали «Сохранить» и узнавали об этом только там. Теперь на вкладке «Настройка» горит янтарный значок, и галочка внизу желтеет вместе с ним.',
      { h: 'Конструктор: обмены оружия, о которых список не знал' },
      'Raptor, взявший мутацию, оставлял себе пиломеч, на который её менял. А конструктор предлагал две мутации там, где на обмен оставался один меч. Так же читались 29 обменов в 17 отрядах: kill team’ы Deathwatch, Death Korps of Krieg, Noise Marines и другие. Теперь отданное оружие уходит из списка, и обменять больше, чем отряд несёт, нельзя.',
      { h: 'Конструктор: на широком экране видно, что вы печатаете' },
      'В светлой теме на широком экране текст в поле названия сливался с фоном. То же было с фракцией, детачментом, очками и заметками рядом. Теперь это обычные поля, и набранное видно.',
      { h: 'Окна больше не отматывают страницу наверх' },
      'Закрыли выбор вторичек у соперника — и страница стояла в самом верху, а его карточка далеко внизу. Так вело себя любое окно в приложении начиная с 2.6.4. Теперь вы остаётесь там, где были.',
      { h: 'Спасибо' },
      'Спасибо всем, кто присылает баг-репорты, и всем, кто предлагает, как сделать лучше. Это очень помогает — многое из того, что выше, выросло из ваших сообщений.',
    ],
  },
  {
    version: '2.6.6',
