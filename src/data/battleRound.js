...
▪ До конца хода, если не указано иное, ваш юнит **не имеет права объявлять нападение** и **начинать задачу (start an action)**.`,
      },
      {
        title: 'Отход',
        body: `◈ МАКСИМАЛЬНОЕ РАССТОЯНИЕ | Характеристика **M** вашего юнита.
...
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит **не имеет права начинать задачу (eligible to start an action)**.`,
  ...
  // 11 — Фаза нападения
  {
    title: 'Фаза нападения (Charge Phase)',
    ...
    subsections: [
      {
        title: 'Шаги фазы нападения (Charge Phase Steps)',
...
▪ 3. Конец фазы нападения`,
      ...
      {
        title: 'Начало фазы нападения (Start of Charge Phase)',
        body: `Правила, срабатывающие в начале фазы нападения, выполняются сейчас.`,
        note: 'Проваленные нападения (Failed Charges): Обратите внимание, что при отсутствии модификаторов броска нападения (charge roll) результат 2 (две единицы) никогда недостаточен для завершения нападения юнитом (charge move), так как юнит не может находиться в радиусе связывания боем (2") при попытке совершить нападение (charge). Такой бросок привёл бы к провалу нападения (failed charge), и юнит не переместился бы.',
      },
      {
...
2. Сделайте бросок на нападение (Make Charge Roll): Сделайте бросок на нападение, бросив 2D6: результат является **максимальным расстоянием** для перемещения нападением (charge move).
3. Попытка нападения (Attempt Charge): Если возможно совершить перемещение нападением (charge move) и если вы всё ещё хотите этого, совершите перемещение нападением этим юнитом. В противном случае ваш юнит не совершает перемещение нападением. В любом случае нападение затем завершается.`,
      },
      {
      ...
      {
        title: 'Перемещение нападением (Charge Move)',
...
◈ ЭФФЕКТ | Ваш юнит перемещается, как описано в разделе «Перемещение» (03).
◈ ПЕРЕД ПЕРЕМЕЩЕНИЕМ | Выберите одного или нескольких вражеских юнитов, которые находятся в пределах 12" от вашего юнита и в пределах **максимального расстояния** от вашего юнита; до конца этого перемещения каждый из этих вражеских юнитов является **целью нападения (charge target)**.
◈ ВО ВРЕМЯ ПЕРЕМЕЩЕНИЯ
▪ Каждая модель должна завершить своё перемещение ближе к одной или нескольким **целям нападения (charge targets)**.
▪ Каждая модель, которая может завершить своё перемещение в пределах 1" от одной или нескольких **целей нападения (charge targets)**, должна сделать это.
▪ Каждая модель, которая может завершить своё перемещение **в бою (engaged)** с одной или несколькими **целями нападения (charge targets)**, должна сделать это.
◈ ПОСЛЕ ПЕРЕМЕЩЕНИЯ
...
▪ Он **связан боем (engaged)**.
▪ Он совершил **перемещение нападением (charge move)** в этом ходу.
▪ Он был выбран для **нападения к врагу (overrun fight)** в этой фазе (12.06).
◈ ЭФФЕКТ | Ваш юнит перемещается, как описано в разделе «Перемещение» (03).
