...
[img:/images/command/battle-shock-examples-3.png|Battle shock example 3 — multiple units in range]
[img:/images/command/battle-shock-examples-4.png|Battle shock example 4 — OC reduction]
`,
...
[img:/images/charge/making-a-charge-move.png|Making a charge move — valid and invalid examples]`,
      },
    ],
  },
  {
...
[img:/images/fight/start-of-fight-phase.png|Fight phase order — fights first and fights last]`,
      },
      {
        id: 'section-12-02',
        sectionNum: '12.02',
...
[img:/images/fight/pile-in-moves.png|Pile-in moves — engaging with enemy units]`,
      },
      {
        id: 'section-12-04',
        sectionNum: '12.04',
...
[img:/images/fight/normal-fight.png|Normal fight — eligible conditions]`,
      },
      {
        id: 'section-12-06',
        sectionNum: '12.06',
...
[img:/images/fight/objective-consolidation.png|Objective consolidation move diagram]`,
 ...
 ru: [
  ...
  {
    title: 'Боевой раунд',
    description: 'Игры в Warhammer 40,000 играются серией игровых раундов. В этом разделе описывается структура игрового раунда, определяющая порядок, в котором игроки делают ходы, чтобы перемещать свои юниты и совершать ими атаки.',
    subsections: [
      {
        title: 'Структура игрового раунда',
        body: `Каждый игровой раунд совершается путём выполнения следующих шагов:
▪ 1. Начало игрового раунда
▪ 2. Ходы игроков
▪ 3. Конец игрового раунда`,
      },
      {
        title: 'Начало игрового раунда',
        body: `В начале игрового раунда игроки выполняют правила, которые срабатывают в начале игрового раунда, прежде чем перейти к ходам игроков.`,
      },
      {
        title: 'Ходы игроков',
        body: `Теперь оба игрока делают по одному ходу. Один и тот же игрок всегда делает первый ход в каждом игровом раунде — миссия, по которой вы играете, подскажет вам, какой именно игрок. После того как ход этого игрока закончился, его оппонент делает свой ход.
...
[img:/images/turn/END-OF-TURN-STEP-RU.png|Иконка фазы: Конец хода]`,
      ...
      {
        title: 'Конец игрового раунда',
        body: `Правила, которые вступают в силу в конце игрового раунда, выполняются в следующем порядке:
...
Затем игровой раунд заканчивается и, если битва не заканчивается, начинается следующий игровой раунд. Миссия, по которой вы играете, подскажет вам, сколько игровых раундов нужно отыграть до окончания битвы.`,
      },
    ],
  },
  // 08 — Фаза командования
  {
    ...
    description: 'В фазе командования оба игрока получают базовые СР, затем вы проверяете боевой дух своих сил и используете любые способности стратегического уровня, которые у вас могут быть.',
    subsections: [
      {
        title: 'Шаги фазы командования',
        body: `Фаза командования происходит путём выполнения следующих шагов:
▪ 1. Начало фазы командования
▪ 2. Получение базовых СР
▪ 3. Шок (Battle-Shock)
...
▪ 5. Конец фазы командования`,
      ...
      {
        title: 'Получение базовых СР',
        body: `Оба игрока получают 1 Очко Командования (CP).`,
        note: 'Очки Командования (Command Points) — это ценный ресурс, который вы можете тратить на использование стратагем (15). Этап получения базовых СР гарантирует, что оба игрока получают по 1 СР в каждый ход. Хотя здесь они называются базовыми (Core) СР, это такие же очки действий, как и любые другие. Некоторые правила иногда упоминают «ОД» (Core CP), когда говорят об этих очках действий.',
      },
      {
...
[img:/images/command/battle-shock-examples-ru.png|Тест боевого шока, пример 1 — провал броска]
[img:/images/command/battle-shock-examples-2-ru.png|Тест боевого шока, пример 2 — юнит с Starting Strength]
[img:/images/command/battle-shock-examples-3-ru.png|Тест боевого шока, пример 3 — несколько юнитов]
[img:/images/command/battle-shock-examples-4-ru.png|Тест боевого шока, пример 4 — снижение OC]
`,
      },
      {
...
2. Затем оба игрока сверяются со своей миссией; если один или оба игрока достигли каких-либо аспектов своей миссии, которые срабатывают в этот момент, выполните их сейчас.`,
  ...
  // 09 — Фаза передвижения
  {
    title: 'Фаза передвижения',
    description: 'В фазе перемещения вы сможете перемещать каждый из своих отрядов по полю боя. Подкрепления также могут прибыть в эту фазу, чтобы усилить ваши силы.',
    subsections: [
      {
        title: 'Шаги фазы передвижения',
        body: `Фаза передвижения осуществляется путём выполнения следующих шагов:
▪ 1. Начало фазы передвижения
▪ 2. Перемещение юнитов
▪ 3. Конец фазы передвижения`,
        note: 'Выбор юнитов для перемещения: Когда шаг «Перемещение юнитов» заканчивается, активный игрок должен выбрать все свои юниты для совершения перемещения, включая находящихся в стратегических резервах (strategic reserves). Это означает, что до окончания фазы каждый юнит в его армии будет выбран для совершения перемещения, даже если он должен оставаться неподвижным (remain stationary).',
      },
      {
        title: 'Начало фазы передвижения',
        body: `Правила, срабатывающие в начале фазы передвижения, выполняются сейчас.`,
      },
      {
...
▪ Fall-back move (Отход) (см.далее)
▪ Disembark move (Высадка) (18.04)
▪ Ingress move (Вторжение) (20.04)`,
      },
      {
        title: 'Конец фазы передвижения',
        body: `Правила, срабатывающие в конце фазы передвижения, выполняются сейчас.`,
      },
      {
        title: 'Оставаться неподвижным',
        body: `◈ МАКСИМАЛЬНОЕ РАССТОЯНИЕ | –
◈ ПРАВО НА ВЫБОР | Любой юнит.
◈ ЭФФЕКТ | Ни одна модель не перемещается (ни по прямой, ни поворачивается). Юниты, которые **остаются неподвижными (remain stationary)**, не активируют правила, которые срабатывают, когда юнит начинает или заканчивает перемещение.`,
      },
      {
        title: 'Обычное перемещение',
...
◈ ПОСЛЕ ПЕРЕМЕЩЕНИЯ | Ваш юнит должен быть **не связан боем (unengaged)**.`,
      },
      {
        title: 'Ускоренное передвижение',
        body: `◈ МАКСИМАЛЬНОЕ РАССТОЯНИЕ | Бросок на ускоренное передвижение (advance roll) + характеристика **M** вашего юнита.
◈ ПРАВО НА ВЫБОР | Ваш юнит находится на поле боя и **не связан боем (unengaged)**.
◈ ЭФФЕКТ | Ваш юнит перемещается, как описано в разделе «Перемещение» (03).
◈ ПЕРЕД ПЕРЕМЕЩЕНИЕМ | Сделайте **бросок на ускоренное передвижение (advance roll)**: бросьте один D6.
◈ ПОСЛЕ ПЕРЕМЕЩЕНИЯ
▪ Ваш юнит должен быть **не связан боем (unengaged)**.
▪ До конца хода, если не указано иное, ваш юнит **не имеет права объявлять рывок** и **начинать задачу (start an action)**.`,
      },
      {
        title: 'Отход',
        body: `◈ МАКСИМАЛЬНОЕ РАССТОЯНИЕ | Характеристика **M** вашего юнита.
...
◈ ЭФФЕКТ | Ваш юнит перемещается, как описано в разделе «Перемещение» (03).
◈ ПЕРЕД ПЕРЕМЕЩЕНИЕМ | Выберите режим отхода:
...
▪ **Отчаянный побег:** Если ваш юнит не подвержен **боевому шоку**, вы должны сделать **проверку боевого шока (battle-shock roll)** для вашего юнита (01.07).`,
        note: '**ВЫБОР РЕЖИМОВ**\nНекоторые правила предписывают вам выбирать режим, например, для отхода (fall-back moves) (09.07). Режимы взаимоисключающие, и вы должны оценивать каждый в представленном порядке. При совершении перемещения, если ваш юнит не соответствует условиям ни одного из режимов, он не может совершить это перемещение.\n\nИногда режим является обязательным, если применим (например, режимы закрепления (consolidation modes) (12.08)), но в случае отхода (fall-back moves) упорядоченное отступление (ordered retreat) не является обязательным, поэтому вы можете вместо него выбрать отчаянный побег (desperate escape).\n\nМногие типы перемещений (move types) указывают условия, которые должны быть соблюдены во время/после перемещения. Те, которые помечены названием режима, применяются только если вы выбрали этот режим; те, которые не помечены названием режима, применяются всегда.',
      },
    ],
  },
  // 10 — Фаза стрельбы
...
▪ Indirect shooting (Непрямая стрельба) (10.07)`,
      ...
      },
      {
        ...
        body: `◈ ПРАВО НА ВЫБОР | Ваш юнит **не связан боем (unengaged)** и не совершал **ускоренное передвижение (advance move)** в этом ходу.
◈ ЭФФЕКТ | Ваш юнит стреляет, как описано в разделе «Совершение атак» (04).
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит **не имеет права начинать задачу (eligible to start an action)**.`,
      },
      {
        title: 'Штурмовая стрельба',
        body: `◈ ПРАВО НА ВЫБОР | К вашему юниту применяется всё следующее:
▪ **Не связан боем (unengaged)** и совершил **ускоренное передвижение (advance move)** в этом ходу.
...
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит **не имеет права начинать задачу (eligible to start an action)**.`,
      },
      {
        ...
        body: `◈ ПРАВО НА ВЫБОР | К вашему юниту применяется всё следующее:
▪ **Связан боем (engaged)** и не совершал **ускоренное передвижение (advance move)** в этом ходу.
...
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит **не имеет права начинать задачу (eligible to start an action)**.`,
      },
      {
        ...
        body: `◈ ПРАВО НА ВЫБОР | К вашему юниту применяется всё следующее:
▪ **Не связан боем (unengaged)** и не совершал **ускоренное передвижение (advance move)** в этом ходу.
...
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит **не имеет права начинать задачу (eligible to start an action)**.`,
  ...
  // 11 — Фаза рывков
  {
    title: 'Фаза рывков (Charge Phase)',
    description: 'В фазе рывков вы сможете совершать рывки (charge moves) своими отрядами. Это отражает то, как ваши силы сокращают дистанцию с врагами, чтобы вступить с ними в кровавый ближний бой.',
    subsections: [
      {
        title: 'Шаги фазы рывков (Charge Phase Steps)',
        body: `Фаза рывков выполняется в следующем порядке:
▪ 1. Начало фазы рывков
▪ 2. Рывок
▪ 3. Конец фазы рывков`,
      },
      {
        title: 'Начало фазы рывков (Start of Charge Phase)',
        body: `Правила, срабатывающие в начале фазы рывков, выполняются сейчас.`,
        note: 'Проваленные рывки (Failed Charges): Обратите внимание, что при отсутствии модификаторов броска рывка (charge roll) результат 2 (две единицы) никогда недостаточен для завершения рывка юнитом (charge move), так как юнит не может находиться в радиусе связывания боем (2") при попытке совершить рывок (charge). Такой бросок привёл бы к провалу рывка (failed charge), и юнит не переместился бы.',
      },
      {
        title: 'Рывок (Charge)',
        body: `Активный игрок выполняет **рывки (charges)** своими имеющими право юнитами по одному, используя последовательность ниже, пока все юниты, которыми он решил совершить рывок, не **объявят рывок (declared a charge)** и эти рывки не будут выполнены.

1. Объявить рывок (Declare Charge): Выберите один союзный юнит, который ещё **не объявлял рывок (not declared a charge)** в этой фазе и **имеет право объявить рывок (eligible to declare a charge)** (см. ниже). Этот юнит объявляет рывок.
Юнит имеет право объявить рывок, если он находится на поле боя, если не указано иное. Вот некоторые правила, которые лишают юнит **права объявить рывок (eligible to declare a charge)**:
...
3. Попытка рывка (Attempt Charge): Если возможно совершить перемещение рывком (charge move) и если вы всё ещё хотите этого, совершите перемещение рывком этим юнитом. В противном случае ваш юнит не совершает перемещение рывком. В любом случае рывок затем завершается.`,
      ...
      {
        title: 'Конец фазы рывков (End of Charge Phase)',
        body: `Правила, срабатывающие в конце фазы рывков, выполняются сейчас.`,
      },
      {
        title: 'Перемещение рывком (Charge Move)',
...
▪ Каждая модель, которая может завершить своё перемещение **в бою (engaged)** с одной или несколькими **целями рывка (charge targets)**, должна сделать это.
◈ ПОСЛЕ ПЕРЕМЕЩЕНИЯ
▪ Ваш юнит должен быть **в бою (engaged)** со всеми **целями рывка (charge targets)**.
...
[img:/images/charge/making-a-charge-move-ru.png|Ход атаки — допустимые и недопустимые примеры]`,
      },
    ],
  },
  // 12 — Фаза ближнего боя
  {
    ...
    description: 'В фазе ближнего боя действуют оба игрока. Сначала юниты сближаются, чтобы максимизировать количество моделей, связанных боем, затем все участники наносят удары в ближнем бою, после чего юниты закрепляются на своих позициях.',
    subsections: [
      {
...
▪ 5. Конец фазы ближнего боя`,
        note: 'Должны ли юниты сражаться? Да, вы должны сражаться всеми юнитами, которые могут, но вы не обязаны сближаться (pile in) или закрепляться (consolidate) с юнитом, если не хотите.',
      },
      {
        title: 'Начало фазы ближнего боя',
        body: `Правила, срабатывающие в начале фазы ближнего боя, выполняются сейчас.

[img:/images/fight/start-of-fight-phase-ru.png|Порядок фазы боя — бьёт первым и бьёт последним]`,
      },
      {
        title: 'Сближение (Pile In)',
        body: `Оба игрока совершают **перемещения сближения (pile-in moves)** (см. ниже) со своими имеющими право юнитами, которые они решают переместить. Игрок, чей сейчас ход, выполняет все свои перемещения первым, затем его оппонент. Каждый юнит не может совершить более одного **перемещения сближения (pile-in move)** во время этого шага.`,
      },
      {
        title: 'Перемещение сближения',
...
▪ Он **связан боем (engaged)**.
▪ Он совершил **перемещение рывком (charge move)** в этом ходу.
▪ Он был выбран для **рывка к врагу (overrun fight)** в этой фазе (12.06).
◈ ЭФФЕКТ | Ваш юнит перемещается, как описано в разделе «Перемещение» (03).
◈ ПЕРЕД ПЕРЕМЕЩЕНИЕМ | Выберите цели сближения (pile-in targets):
...
▪ Каждая перемещаемая модель должна завершить своё перемещение ближе к ближайшей **цели сближения (pile-in target)** и **связанной боем (engaged)** с ней, если возможно.
◈ ПОСЛЕ ПЕРЕМЕЩЕНИЯ
...
[img:/images/fight/pile-in-moves-ru.png|Перемещение в контакт — вступление в бой с врагом]`,
      },
      {
        title: 'Бой',
...
[img:/images/fight/normal-fight-ru.png|Обычный бой — условия доступности]`,
      },
      {
        title: 'Рывок к врагу',
...
[img:/images/fight/overrun-fight-ru.png|Бой с преследованием — условия доступности]`,
      ...
      {
        title: 'Закрепление (Consolidate)',
        body: `Оба игрока совершают **перемещения закрепления (consolidation moves)** (см. ниже) со своими имеющими право юнитами, которые они решают переместить. Игрок, чей сейчас ход, выполняет все свои перемещения первым, затем его оппонент. Каждый юнит не может совершить более одного **перемещения закрепления (consolidation move)** во время этого шага.`,
      },
      {
        title: 'Перемещение закрепления',
        body: `◈ МАКСИМАЛЬНОЕ РАССТОЯНИЕ | 3"
◈ ПРАВО НА ВЫБОР | Идёт фаза ближнего боя, и ваш юнит **имел право сражаться (eligible to fight)** в этой фазе.
◈ ЭФФЕКТ | Ваш юнит перемещается, как описано в разделе «Перемещение» (03).
◈ ПЕРЕД ПЕРЕМЕЩЕНИЕМ | Выберите режим закрепления (consolidation mode):
▪ **Продолжающееся закрепление (Ongoing Consolidation):** Если ваш юнит **связан боем (engaged)**, вы должны выбрать этот режим и выбрать каждый вражеский юнит, с которым он **связан боем**.
▪ **Боевое закрепление (Engaging Consolidation):** В противном случае, если ваш юнит находится в пределах 3" от одного или нескольких вражеских юнитов, вы должны выбрать этот режим и выбрать одного или нескольких из этих вражеских юнитов.
▪ **Закрепление на цели (Objective Consolidation):** В противном случае, если ваш юнит находится в пределах 3" от одной или нескольких целей (objectives), вы должны выбрать этот режим и выбрать одну из этих целей.
◈ ВО ВРЕМЯ ПЕРЕМЕЩЕНИЯ
▪ **Продолжающееся закрепление:** Модели в базовом контакте с одной или несколькими вражескими моделями не могут быть перемещены. Каждая перемещаемая модель должна завершить своё перемещение ближе к ближайшему выбранному вражескому юниту и **связанной боем (engaged)** с ним, если возможно.
▪ **Боевое закрепление:** Каждая перемещаемая модель должна завершить своё перемещение ближе к ближайшему выбранному вражескому юниту и **связанной боем (engaged)** с ним, если возможно.
▪ **Закрепление на цели:** Каждая перемещаемая модель должна завершить своё перемещение в пределах досягаемости выбранной цели (objective), если возможно, или ближе к ней, если нет.
◈ ПОСЛЕ ПЕРЕМЕЩЕНИЯ
▪ **Продолжающееся закрепление:** Каждая модель, которая начала это перемещение **связанной боем (engaged)** с вражеским юнитом, должна всё ещё быть **связана боем (engaged)** с этим вражеским юнитом.
...
[img:/images/fight/ongoing-consolidation-ru.png|Диаграмма консолидации с вступлением в контакт]

[img:/images/fight/objective-consolidation-ru.png|Диаграмма консолидации к цели]`,
