      ...
      subsections: [
        {
          ...
          body: `Before the battle, place a selection of **[gloss:terrain-feature:terrain features]** on the battlefield using one or more of the following methods:
...
[img:/images/terrain/terrain-placed-on-the-battlefield.jpg|Terrain on the battlefield — terrain area boundary example]`,
        },
        {
          ...
          body: `Each **terrain feature** belongs to a **[gloss:terrain-category:terrain category]**, which can affect the movement and visibility of models. Below are some examples of which types of **terrain features** fall under each category.
          ...
          children: [
            {
              ...
              fromApp: true,
              body: `Don't worry if your battlefield doesn't match a mission's **terrain category** requirements; just do the best with the **terrain features** you have. If a mission does not provide you with guidance, it will be up to the players to decide how to create their battlefield. When doing so, keep in mind that **[gloss:dense-terrain:dense terrain features]** have the greatest impact on visibility and movement. Battlefields with too few **dense terrain features** can advantage armies that rely on shooting, or disadvantage armies that rely on melee. Also remember to leave sufficient room around **dense terrain features** for larger models such as MONSTERS/VEHICLES to manoeuvre, especially near the edges of the battlefield.`,
            },
          ],
        },
        {
...
While a model is **hidden**, it can only be **[gloss:visible:visible]** to enemy models that are within its **[gloss:detection-range:detection range]**. Unless otherwise stated, a model's **detection range** is 15".`,
          children: [
            {
              id: 'section-13-09-01',
              sectionNum: '13.09.01',
      ...
      subsections: [
        {
          ...
          body: `During the battle, both players can use **[gloss:stratagem:stratagems]**. Each **stratagem** states:
...
◈ AFTER SHOOTING | Until the end of the phase, your unit is not **[gloss:eligible-to-act:eligible to start an action]**.`,
      ...
      stratagems: [
        ...
        },
        {
          ...
          effect: 'Resolve a **[gloss:charge:charge]** with your unit (11.02). While doing so, before making the **charge roll**, you must select one of the following modes:\n▪ **Leap to Defend:** When selecting **[gloss:charge-target:charge targets]**, you can only select enemy units that made a **charge move** this phase and are within the **[gloss:maximum-distance:maximum distance]**.',
          restrictions: '',
          extraCost: [
            {
              cp: '+1CP',
      ...
      subsections: [
        {
          ...
          body: `Some rules allow units to perform **[gloss:action:actions]**. Each **action** states:
      ...
      subsections: [
        {
          ...
          body: `Перед битвой разместите набор **[gloss:terrain-feature:элементов укрытий]** на поле боя, используя один или несколько из следующих методов:
...
[img:/images/terrain/terrain-placed-on-the-battlefield-ru.jpg|Укрытия на поле боя — пример границы территории]`,
        },
        {
          ...
          body: `Каждый **[gloss:terrain-feature:элемент укрытия]** принадлежит к **[gloss:terrain-category:категории укрытий]**, которая может влиять на движение и видимость моделей. Ниже приведены примеры того, какие типы **элементов укрытий** попадают под каждую категорию.

Карта развёртывания миссии может определять, какие **категории укрытий** должны присутствовать в каждой **[gloss:terrain-area:области укрытия]**. Соблюдение этих требований создаст наилучший игровой опыт.`,
          note: 'Обратите внимание, что **[gloss:terrain-feature:элементы укрытий]**, находящиеся в одной и той же **[gloss:terrain-area:области укрытия]**, могут принадлежать к разным **[gloss:terrain-category:категориям укрытий]**.',
          children: [
            {
              title: 'Создание собственного поля боя',
              body: `Не переживайте, если ваше поле боя не соответствует требованиям миссии к **[gloss:terrain-category:категориям укрытий]**; просто используйте имеющиеся **[gloss:terrain-feature:элементы укрытий]** как можно лучше. Если миссия не даёт вам указаний, игроки сами решают, как создать своё поле боя. При этом помните, что **[gloss:dense-terrain:надёжные элементы укрытий]** сильнее всего влияют на видимость и движение. Поля боя со слишком малым числом **надёжных элементов укрытий** могут давать преимущество армиям, полагающимся на стрельбу, или ставить в невыгодное положение армии, полагающиеся на ближний бой. Также не забудьте оставить достаточно места вокруг **надёжных элементов укрытий** для манёвра крупных моделей, таких как MONSTERS/VEHICLES, особенно у краёв поля боя.`,
            },
          ],
        },
        {
...
**Примеры:** Здания, руины, бронированные контейнеры, леса.`,
        },
        {
          ...
          body: `Модели могут перемещаться через различные категории **[gloss:terrain-feature:элементов укрытий]** следующим образом:
...
Пока модель находится в **скрытности**, она может быть **[gloss:visible:видна]** только вражеским моделям, которые находятся в пределах её **[gloss:detection-range:радиуса обнаружения]**. Если не указано иное, **радиус обнаружения** модели составляет 15".`,
          children: [
            {
      ...
      subsections: [
        {
          ...
          body: `Если в миссии используются **[gloss:objective:цели]**, в ней указывается, где они находятся на поле боя. Как правило, в вашей миссии будет карта развёртывания, показывающая несколько точек, в которых должны быть размещены **цели**. Расположение каждой точки должно совпадать с **[gloss:terrain-area:областью укрытия]** (13.01); этот **[gloss:terrain-feature:элемент укрытия]** является **целью** и называется **целевым объектом местности** (terrain objective).
...
Модель находится в пределах досягаемости **маркера цели**, пока она находится в пределах 3" по горизонтали и 5" по вертикали от этого **маркера цели**. При измерении расстояний до **маркера цели** и от него измеряйте до ближайшей его части.`,
        ...
        },
        {
          ...
          body: `В начале битвы ни одна **[gloss:objective:цель]** на поле боя не находится под контролем кого-либо из игроков. Чтобы получить контроль над **целью**, игроку потребуется одна или несколько моделей с характеристикой **OC** 1 или более в пределах досягаемости до неё. Модель находится в пределах досягаемости до **целевого объекта местности** (terrain objective), пока она находится в пределах этой **[gloss:terrain-area:области укрытия]**.
      ...
      subsections: [
        {
          ...
          body: `В ходе битвы оба игрока могут использовать **[gloss:stratagem:стратегемы]**. Каждая **стратегема** указывает:
          ...
          children: [
            {
              ...
              body: `Некоторые правила изменяют стоимость использования определённых **[gloss:stratagem:стратегем]**. В таких случаях применяются следующие положения:
...
▪ Если правило изменяет стоимость **стратегемы** в CP, когда вы её используете, но не указывает продолжительность этого модификатора, этот модификатор применяется только к этому использованию этой **стратегемы**.`,
            },
            {
              ...
              body: `Если отряд выбран целью **[gloss:stratagem:стратегемы]**, говорят, что он подвержен этой **стратегеме**.`,
            },
            {
              title: 'Определения стратегем',
              body: `▪ **Стратегемы вашего оппонента (Your Opponent's Stratagems):** **[gloss:stratagem:стратегемы]**, которые использует ваш оппонент, иногда называются стратегемами вашего оппонента.
...
▪ **Ваш отряд (в стратегемах) (Your Unit (in Stratagems)):** Термин «ваш отряд(ы)» в разделе «Эффект» (Effect) **стратегем** всегда относится к отряду(ам), выбранному в разделе «Цель» (Target) этой **стратегемы**, и никогда не относится к каким-либо другим отрядам из вашей армии.`,
        ...
        },
        {
          ...
          body: 'Все игроки могут использовать **[gloss:stratagem:базовые стратегемы]** (см. далее). Дополнительные **стратегемы** можно найти в кодексах и других публикациях.',
        },
        {
          id: 'section-15-09',
          sectionNum: '15.09',
...
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит не может **[gloss:action:начать действие]**.`,
      ...
      stratagems: [
        {
          ...
          effect: 'Вы перебрасываете этот бросок. Если вы бросаете несколько костей одновременно, выберите одну из этих костей для переброса (за исключением **[gloss:charge-roll:бросков на нападение]**, которые должны перебрасываться полностью).',
          restrictions: '',
          extraCost: [],
        },
        {
          flavor: 'Легенды 41-го тысячелетия изобилуют рассказами о смертельных поединках между могущественными героями.',
          when: 'Фаза ближнего боя, сразу после того, как дружественный юнит ПЕРСОНАЖА выбран для совершения атак.',
        ...
        },
        {
          ...
          when: 'Ваша фаза нападения (Charge phase), сразу после того, как дружественный юнит MONSTER/VEHICLE (МОНСТРА/ТРАНСПОРТА) завершает **[gloss:charge-move:манёвр нападения]**.',
          target: 'Этот юнит MONSTER/VEHICLE.',
          effect: 'Отыграйте следующую последовательность действий:\n1. Выберите один вражеский юнит, **[gloss:engaged:связанный ближним боем]** с вашим юнитом.\n2. Выберите одну модель в вашем юните, **связанную ближним боем** с этим вражеским юнитом.\n3. Бросьте количество D6, равное характеристике **T** этой модели: за каждый результат 1 ваш юнит получает 1 **[gloss:mortal-wound:смертельную рану]**; за каждый результат 5+ этот вражеский юнит получает 1 **смертельную рану** (максимум 6 **смертельных ран** за юнит).',
          restrictions: '',
          extraCost: [],
        },
        {
        ...
        },
        {
          ...
          effect: 'До конца фазы ваш юнит получает способность **[gloss:fights-first:«Атакует первым»]** и он должен быть следующим юнитом, которого вы выбираете [gloss:selected-to-fight:для совершения атак] (12.04).',
          restrictions: '',
          extraCost: [],
        },
      ],
    },
    {
      ...
      title: 'Действия',
      page: 58,
      description: '[gloss:action:Действия] представляют собой жизненно важные боевые задачи, которые должны выполнить ваши войска: от поиска утерянного археотеха до завершения кощунственного ритуала.',
      subsections: [
        {
          id: 'section-16-01',
          sectionNum: '16.01',
