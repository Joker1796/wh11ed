...
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит не может **начать задание** (start an action).`,
      ...
      stratagems: [
        ...
        },
        {
          ...
          when: 'Ваша фаза нападения (Charge phase), сразу после того, как дружественный юнит MONSTER/VEHICLE (МОНСТРА/ТРАНСПОРТА) завершает **перемещение нападением** (charge move).',
        ...
        },
        {
          ...
          effect: 'Совершите **нападение** (charge) вашим юнитом (11.02). При этом перед совершением **броска на нападение** (charge roll) вы должны выбрать один из следующих режимов:\n▪ **Прыжок для защиты:** При выборе целей для нападения (charge targets) вы можете выбирать только те вражеские юниты, которые совершили **перемещение нападением** (charge move) в этой фазе и находятся в пределах **максимальной дистанции** (maximum distance).',
          restrictions: '',
          extraCost: [
            {
              cp: '+1CP',
...
### Завершение задания
Если юнит, выполняющий **задание**, совершает перемещение (за исключением **сближения** (pile-in) и **консолидации** (consolidation moves)) или покидает поле боя, этот юнит не завершает это **задание**. В противном случае, когда **задание** завершается, срабатывает его раздел «Эффект».`,
