...
[img:/images/terrain/terrain-placed-on-a-mat-ru.jpg|Укрытие на мате — пример границы территории]

[img:/images/terrain/terrain-placed-on-the-battlefield-ru.jpg|Укрытие на поле боя — пример границы территории]`,
        },
        {
          id: 'section-13-02',
          sectionNum: '13.02',
          title: 'Категории укрытий',
          body: `Каждый **элемент укрытия** (terrain feature) принадлежит к **категории укрытий** (terrain category), которая может влиять на перемещение и видимость моделей. Ниже приведены примеры того, какие типы **элементов укрытий** попадают под каждую категорию.
...
[img:/images/terrain/terrain-and-movement-ru.jpg|Укрытие и движение — перемещение моделей через укрытие]

[img:/images/terrain/terrain-and-movement-2-ru.jpg|Укрытие и движение, пример 2 — правила выноса основания]`,
          note: 'Сплошное укрытие: Если **элемент укрытия** (terrain feature) обладает правилом Сплошное (Solid) (13.11), модель не может завершить перемещение так, чтобы любая её часть оказалась по ту сторону любой закрытой части этого **элемента укрытия**, находящейся на высоте 3" или менее от уровня земли — даже через небольшие проёмы, такие как двери и окна.',
        },
        {
          id: 'section-13-07',
          sectionNum: '13.07',
...
◈ ПОСЛЕ СТРЕЛЬБЫ | До конца фазы ваш юнит не может **начать задание** (start an action).`,
      ...
      stratagems: [
        ...
        },
        {
          ...
          target: 'Один дружественный **не вовлечённый в ближний бой** (unengaged) юнит EXPLOSIVES/GRENADES (ВЗРЫВЧАТКА/ГРАНАТЫ), который является допустимой целью для стрельбы и не совершал **ускор. передвижение** (advance move) в этом ходу.',
          effect: 'Выполните следующую последовательность действий:\n1. Выберите одну модель EXPLOSIVES/GRENADES в вашем юните.\n2. Выберите один **не вовлечённый в ближний бой** вражеский юнит в пределах 8" от этой модели и в зоне её видимости (visible).\n3. Бросьте шесть D6: за каждый результат 4+ этот вражеский юнит получает 1 **неотвратимую рану** (mortal wound).',
          restrictions: '',
          extraCost: [],
        },
        {
          ...
          when: 'Ваша фаза рывка (Charge phase), сразу после того, как дружественный юнит MONSTER/VEHICLE (МОНСТРА/ТРАНСПОРТА) завершает **перемещение рывком** (charge move).',
          target: 'Этот юнит MONSTER/VEHICLE.',
          effect: 'Выполните следующую последовательность действий:\n1. Выберите один вражеский юнит, **связанный ближним боем** (engaged) с вашим юнитом.\n2. Выберите одну модель в вашем юните, **связанную ближним боем** с этим вражеским юнитом.\n3. Бросьте количество D6, равное характеристике **T** этой модели: за каждый результат 1 ваш юнит получает 1 **неотвратимую рану** (mortal wound); за каждый результат 5+ этот вражеский юнит получает 1 **неотвратимую рану** (максимум 6 **неотвратимых ран** за юнит).',
          restrictions: '',
          extraCost: [],
        },
        {
        ...
        },
        {
          ...
          when: 'Конец фазы рывка (Charge phase) вашего противника.',
          ...
          effect: 'Совершите **рывок** (charge) вашим юнитом (11.02). При этом перед совершением **броска на рывок** (charge roll) вы должны выбрать один из следующих режимов:\n▪ **Прыжок для защиты:** При выборе целей для рывка (charge targets) вы можете выбирать только те вражеские юниты, которые совершили **перемещение рывком** (charge move) в этой фазе и находятся в пределах **максимальной дистанции** (maximum distance).',
          restrictions: '',
          extraCost: [
            {
              cp: '+1CP',
              title: 'В гущу схватки',
              body: 'При совершении **броска на рывок** (charge roll), если его результат (после модификаторов) больше 6, измените его на 6. При выборе целей для рывка (charge targets) вы можете выбирать любые вражеские юниты, которые находятся в пределах 6" от вашего юнита и в пределах **максимальной дистанции** (maximum distance).',
            },
          ],
        },
        {
          flavor: 'В рукопашной схватке даже миг промедления даёт быстрому противнику шанс, которым он непременно воспользуется.',
          when: 'Шаг ближнего боя (Fight step) фазы ближнего боя (Fight phase) вашего противника, сразу после того, как вражеский юнит завершил свои атаки.',
      ...
      subsections: [
        {
          ...
          body: `Некоторые правила позволяют юнитам выполнять **задания** (actions). Каждое **задание** указывает:
...
### Начало задания
Юнит **может начать задание** (eligible to start an action), если к нему **не** применяются одно или несколько из следующих условий:
...
▪ Он совершил **ускор. передвижение** (advance move) или **отступление** (fall-back move) в этом ходу.
...
▪ Он **не может объявлять рывок** (not eligible to declare a charge).

### Завершение задания
Если юнит, выполняющий **задание**, совершает перемещение (за исключением **сближения** (pile-in) и **закрепления** (consolidation moves)) или покидает поле боя, этот юнит не завершает это **задание**. В противном случае, когда **задание** завершается, срабатывает его раздел «Эффект».`,
          note: 'ГДЕ НАЙТИ ЗАДАНИЯ\n**Задания** (actions), которые могут использовать игроки, можно найти в других публикациях, таких как пакеты миссий. Все игроки могут использовать **базовые задания** (core actions), тогда как другие **задания** могут иметь ограничения, например по фракции армии (army faction).',
        },
      ],
    },
  ],
