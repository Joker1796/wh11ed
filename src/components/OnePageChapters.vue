<template>
  <div class="view">
    <div class="view-hero">
      <h1>{{ heading }}</h1>
      <p class="view-hero-desc">
        {{ desc }}
      </p>
    </div>

    <ChapterToc
      :groups="tocGroups"
      :active-id="activeId"
      @select="goToAnchor"
    />

    <section
      v-for="chapter in chapters"
      :id="chapter.id"
      :key="chapter.id"
      class="book-chapter"
    >
      <component :is="chapter.component" />
    </section>

    <!-- Desktop FAB, stacked above the slot App.vue's BackToTopButton occupies, and shown
         only alongside it (same scroll threshold) — at the top of the page there's nothing
         to jump back up to yet. Mobile gets the same action through MobileUtilityBar
         (contributed below) — no third floating element fighting for the corner. A plain
         reactive class (not v-if + Transition) — this view is itself the child of App.vue's
         routed <Transition mode="out-in">, and a second Transition nested inside it never
         completed its own leave, so the button got stuck visible instead of unmounting. -->
    <button
      type="button"
      class="fab-btn book-toc-fab"
      :class="{ 'book-toc-fab--hidden': !backToTopVisible }"
      :aria-hidden="!backToTopVisible"
      :tabindex="backToTopVisible ? 0 : -1"
      :title="labels.openContents"
      :aria-label="labels.openContents"
      @click="tocOpen = true"
    >
      <i class="bi bi-list-ul" />
    </button>

    <BaseModal
      v-if="tocOpen"
      :title="labels.contentsHeading"
      :max-width="modalWidth"
      @close="tocOpen = false"
    >
      <!-- `modal-body` is not cosmetic: it carries the global `overscroll-behavior: contain`
           that keeps a scroll past the end of the list from chaining to the page behind. -->
      <div class="modal-body">
        <ChapterToc
          variant="modal"
          :groups="tocGroups"
          :active-id="activeId"
          :subsections-for="subsectionsFor"
          @select="onModalSelect"
        />
      </div>
    </BaseModal>
  </div>
</template>

<script setup>
// A book rendered as one page: Core Rules (/core-rules, seven chapters) and the Event Companion
// (/event-companion, eight). Hero, contents, every chapter at once, a contents button once the
// reader has scrolled, a scroll-spy that keeps the contents' highlight on the section being read.
// The two views were one copy apart (94 lines of ~360 differed, all of them names); they now pass
// their chapters and their contents here and keep nothing else.
import { ref, computed, watch, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import BaseModal from './BaseModal.vue'
import ChapterToc from './ChapterToc.vue'
import { ui } from '../i18n/ui.js'
import { useLocale } from '../composables/useLocale.js'
import { useActiveSection } from '../composables/useActiveSection.js'
import { useContributeMobileActions } from '../composables/useMobileActionBar.js'
import { scrollToAnchor } from '../composables/useRefNavigation.js'
import { useBackToTop } from '../composables/useBackToTop.js'

const props = defineProps({
  heading: { type: String, required: true },
  desc: { type: String, default: '' },
  // [{ id, component }] in page order — each chapter's wrapper <section> is drawn here.
  chapters: { type: Array, required: true },
  // The book's localised navGroups (router/index.js) — the subnav, the drawer and these contents
  // are one list.
  groups: { type: Array, required: true },
  // The opening chapter has no sections of its own and sits right above the contents as page
  // content — listing it there would just be a link to "scroll up a bit". Left out of the
  // contents only; it stays a chapter everywhere else (subnav, drawer, search).
  introHash: { type: String, required: true },
  path: { type: String, required: true },
  // Distinct per book: MobileUtilityBar keys its contributions by it.
  actionKey: { type: String, required: true },
  subsectionsFor: { type: Function, default: () => [] },
  // Core's modal carries a third level (NN.MM) and needs the width to lay it out in columns.
  modalWidth: { type: String, default: '46rem' },
})
// `filter`: a section of Core's Reference chapter narrows the ability list it jumps to.
const emit = defineEmits(['filter'])

const { locale } = useLocale()
const route = useRoute()
const router = useRouter()
const labels = computed(() => ui[locale.value])

const tocOpen = ref(false)
const { visible: backToTopVisible } = useBackToTop()
const tocGroups = computed(() => props.groups.filter((g) => g.hash !== props.introHash))

// The anchors the contents list, in document order — the scroll-spy walks exactly these.
const spyIds = computed(() => {
  const ids = []
  for (const g of props.groups) {
    ids.push(g.hash.slice(1))
    for (const s of g.sections) if (!ids.includes(s.id)) ids.push(s.id)
  }
  return ids
})
const { activeId, measure } = useActiveSection(spyIds)

// One entry point for every in-page jump (contents, modal, subnav). The hash goes into the URL
// so the position is shareable and useViewRestore can remember it; scrollToAnchor does the
// actual work — it polls for the element, which is what makes a jump into a chapter that
// `content-visibility` has not laid out yet land in the right place.
async function goToAnchor(id, filter) {
  if (filter) emit('filter', filter)
  if (route.hash !== '#' + id) await router.push({ path: props.path, hash: '#' + id })
  scrollToAnchor(id)
}

function onModalSelect(id, filter) {
  tocOpen.value = false
  goToAnchor(id, filter)
}

// Mobile: the contents button joins the shared utility strip instead of adding another fixed
// element above the bottom nav — shown only once scrolled down, same as the desktop FAB and the
// bar's own back-to-top icon right next to it (both read backToTopVisible/useBackToTop).
useContributeMobileActions(props.actionKey, () => !backToTopVisible.value ? [] : [
  {
    key: props.actionKey,
    icon: 'bi bi-list-ul',
    label: labels.value.openContents,
    onClick: () => { tocOpen.value = true },
  },
])

onMounted(() => {
  if (route.hash) scrollToAnchor(route.hash.slice(1))
  measure()
})

// A chapter/section jump only changes the hash, so the view is never re-created (the RouterView
// key is the path) — re-run the scroll ourselves.
watch(() => route.hash, (hash) => {
  if (hash) scrollToAnchor(hash.slice(1))
})
</script>

<style scoped>
/* Skip layout/paint for chapters that are off screen. `auto` in contain-intrinsic-size
   makes the browser remember each chapter's real height after it has been rendered once,
   so the scrollbar doesn't jump around as the reader moves through the page. Where it's
   unsupported the page just renders in full — the behaviour it had as separate pages. */
.book-chapter {
  content-visibility: auto;
  contain-intrinsic-size: auto 3000px;
  scroll-margin-top: var(--header-total);
}

.book-toc-fab {
  display: none;
  position: fixed;
  right: 1.5rem;
  /* One FAB slot above BackToTopButton (60px tall, bottom 1.5rem) so the two never overlap
     — both share the same scroll threshold (backToTopVisible), so this slot is only
     ever "active" while BackToTopButton itself is showing. */
  bottom: calc(1.5rem + 60px + 0.75rem);
  z-index: 195;
  opacity: 1;
  transform: scale(1);
  transition: opacity var(--motion-fast) ease, transform var(--motion-fast) ease;
}

.book-toc-fab--hidden {
  opacity: 0;
  transform: scale(0.6);
  pointer-events: none;
}

@media (min-width: 901px) {
  .book-toc-fab { display: flex; }
}
</style>
