  ...
  <div>
    ...
    <div class="army-rule-block">
      <div class="army-rule-header">
        ...
        <span class="army-rule-name">{{ necrons.armyRule.name }}</span>
      </div>
      <div class="army-rule-body"><RuleBody :body="necrons.armyRule.text" /></div>
    </div>
    ...
    <div class="det-filter-bar">
      <div class="det-select-wrap" :class="{ open: filterOpen }" @click.stop>
        ...
        <div class="det-dropdown" v-if="filterOpen">
          ...
          >All</button>
          <button
            v-for="det in necrons.detachments"
            :key="det.id"
            class="det-dropdown-item"
...
import { necrons } from '../../data/factions/necrons.js'
...
const filteredDetachments = computed(() =>
  selectedDet.value
    ? necrons.detachments.filter(d => d.id === selectedDet.value)
    : necrons.detachments
)

const selectedDetLabel = computed(() =>
  selectedDet.value
    ? necrons.detachments.find(d => d.id === selectedDet.value)?.name
    : 'All Detachments'
)
